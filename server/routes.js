...
router.post('/transactions', (req, res) => {
    ...
    db.run(
        `INSERT INTO transactions (id, date, type, cost, description, metadata) VALUES (?, ?, ?, ?, ?, ?)`,
        [id, date, type, cost, description, metaStr],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
...
router.post('/datasets', (req, res) => {
    ...
    db.run(
        `INSERT OR REPLACE INTO datasets (id, name, platform, targetProfile, dataType, createdAt, recordCount, project, tags, full_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, name, platform, targetProfile, dataType, createdAt, recordCount, project, tagStr, fullJson],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
