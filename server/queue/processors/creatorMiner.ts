...
export const creatorMinerProcessor = async (job: Job) => {
    ...
    try {
        ...
        if (query.startsWith('@')) {
            ...
            items = result.items.map((post: any) => post.owner || post.author).filter((x: any) => x);
        }
        ...
        const creators = items.map((user: any) => ({
            ...
            .filter((u: any) => u.username && !u.isPrivate)
