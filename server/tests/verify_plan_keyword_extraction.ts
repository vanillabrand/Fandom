...
async function verifyKeywordExtraction() {
    ...
    for (const test of testCases) {
        ...
        try {
            ...
            if (plan.search_keywords) {
                ...
                const match = plan.search_keywords.some((k: string) => k.toLowerCase().includes(test.expected.toLowerCase()) || test.expected.toLowerCase().includes(k.toLowerCase()));
                if (match) console.log("   -> MATCHES EXPECTATION");
                else console.warn(`   -> EXPECTED "${test.expected}", GOT ${JSON.stringify(plan.search_keywords)}`);
            } else {
            ...
            const searchStep = plan.steps.find((s: any) => s.actorId.includes('instagram-scraper') || s.actorId.includes('instagram-api-scraper'));
    ...
    if (uri) await mongoService.disconnect();
}
