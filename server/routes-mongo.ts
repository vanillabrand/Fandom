...
import { mongoService, SupportTicket } from './services/mongoService.js';
...
router.post('/auth/email/signup', async (req: any, res) => {
    try {
        ...
        try {
            ...
            await emailService.sendEmail(
                email,
                'Welcome to Fandom Analytics - Account Created',
                welcomeEmail
            );
...
router.post('/auth/email/login', async (req: any, res) => {
    try {
...
router.post('/auth/login', authMiddleware, async (req: any, res) => {
    try {
        const user = req.user;
...
router.get('/user/balance', authMiddleware, async (req: any, res) => {
    try {
...
router.get('/user/usage', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
...
router.get('/user/transactions', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
...
router.get('/user/usage-history', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
...
// GET /api/datasets
router.get('/datasets', authMiddleware, async (req: any, res) => {
    try {
...
// GET /api/datasets/:id
router.get('/datasets/:id', authMiddleware, async (req: any, res) => {
    try {
        ...
        if (dataset && dataset.data) {
            const graphSnapshot = dataset.data.find((r: any) => r.recordType === 'graph_snapshot');
            if (graphSnapshot && graphSnapshot.data) {
                const gapHandles = (jobOrchestrator as any).identifyEnrichmentGaps(graphSnapshot.data);
                if (gapHandles.length > 0) {
                    // Try to find an associated job
                    const associatedJobs = await mongoService.getJobsByDatasetId(dataset.id);
                    let isEnriching = associatedJobs.some((j: any) => j.metadata?.isEnriching === true) || dataset.isEnriching;

                    if (!isEnriching) {
                        let mainJob = associatedJobs.find((j: any) => j.type === 'map_generation' || j.type === 'enrichment' || j.type === 'orchestration');
                        ...
                        if (!mainJob) {
                            ...
                            mainJob = { id: jobId } as any;
                        }

                        if (mainJob) {
                            console.log(`[DatasetLoadFill] ⚠️ Dataset ${dataset.id} has gaps. Triggering background enrichment...`);
                            const profileMap = new Map();
                            jobOrchestrator.performDeepEnrichment(graphSnapshot.data, dataset.id, mainJob.id, profileMap).catch((err: any) => {
...
router.post('/datasets', authMiddleware, async (req: any, res) => {
    try {
...
router.patch('/datasets/:id', authMiddleware, async (req: any, res) => {
    try {
        const { id } = req.params;
...
router.delete('/datasets/:id', authMiddleware, async (req: any, res) => {
    try {
        const { id } = req.params;
...
router.get('/jobs/:id/proxy-dataset', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
        ...
        const fetchPromises = datasetIds.map(async (dsId: string) => {
            const resp = await fetch(`https://api.apify.com/v2/datasets/${dsId}/items?token=${apifyToken}&limit=5000&clean=true`);
            if (!resp.ok) return []; // Ignore failed/missing datasets for now
            return resp.json();
        });
...
router.post('/jobs', authMiddleware, async (req: any, res) => {
    try {
...
router.get('/jobs/:id', authMiddleware, async (req: any, res) => {
    try {
        const job = await mongoService.getJob(req.params.id);
        if (!job) return res.status(404).json({ error: 'Job not found' });
        ...
        if (anyJob.status === 'completed' && anyJob.result?.analysisResult) {
            const gapHandles = (jobOrchestrator as any).identifyEnrichmentGaps(anyJob.result.analysisResult);
            if (gapHandles.length > 0) {
                ...
                (jobOrchestrator as any).performDeepEnrichment(anyJob.result.analysisResult, datasetId, anyJob.id, profileMap).catch((err: any) => {
                    console.error(`[RuntimeFill] ❌ Background enrichment failed for job ${anyJob.id}:`, err);
                });

                if (!anyJob.metadata) anyJob.metadata = {};
...
router.delete('/jobs/:id', authMiddleware, async (req: any, res) => {
    try {
        const { id } = req.params;
        const job = await mongoService.getJob(id);

        if (!job) return res.status(404).json({ error: 'Job not found' });
        if (job.userId !== req.user.sub) return res.status(403).json({ error: 'Access denied' });
...
router.post('/credits/deduct', authMiddleware, async (req: any, res) => {
    try {
...
// POST /api/support
router.post('/support', authMiddleware, async (req: any, res) => {
    try {
        ...
        await emailService.sendEmail(
            userEmail,
            `Support Ticket ${ticketId} - We'll respond within 24 hours`,
        ...
        await emailService.sendEmail(
            'vanillabrand@googlemail.com',
            `[New Ticket] ${ticketId}: ${subject}`,
...
router.post('/user/redeem', authMiddleware, async (req: any, res) => {
    try {
        const { code } = req.body;
        if (!code) {
        ...
        if (status === 'active') {
            try {
                ...
                if (user && user.email) {
                    ...
                    await emailService.sendEmail(
                        user.email,
                        '✅ Your Fandom Analytics Account is Approved!',
                        approvalEmail
                    );
...
router.get('/admin/jobs', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
...
router.get('/admin/users/:userId/jobs', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
...
router.delete('/admin/maintenance/clear', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        const user = await mongoService.getUser(req.user.sub);
...
router.post('/admin/promo-codes', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
...
router.post('/admin/invoices/generate', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
...
router.post('/admin/invoices/:id/send', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        const { id } = req.params;
...
router.post('/admin/invoices/:id/resend', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        const { id } = req.params;
...
router.patch('/admin/invoices/:id/status', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        const { id } = req.params;
...
router.post('/admin/pricing-config', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
...
router.get('/admin/support', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
...
router.post('/admin/support/:ticketId/reply', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        const { ticketId } = req.params;
...
router.patch('/admin/support/:ticketId/status', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        const { ticketId } = req.params;
        const { status } = req.body;
...
router.get('/admin/analytics', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        ...
        const userAnalytics = await Promise.all(users.map(async (user) => {
            ...
            const breakdown: Record<string, { count: number; revenue: number; cost: number }> = {};
            usage.forEach(log => {
...
router.get('/admin/accuracy-metrics', authMiddleware, adminMiddleware, async (req: any, res) => {
    try {
        ...
        feedbackDocs.forEach(f => {
            f.feedback.categories?.forEach((category: string) => {
...
router.get('/user/analytics/trends', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
...
router.get('/user/analytics/insights', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
...
router.get('/user/analytics/benchmark', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
...
router.post('/user/analytics/roi', authMiddleware, async (req: any, res) => {
    try {
        const {
...
router.post('/payments/create-intent', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
        const userEmail = req.user.email;
...
router.post('/payments/confirm', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
        ...
        const result = await mongoService.processPaymentBalance(
            paymentIntentId,
            userId,
            details.amountGBP
        );
...
router.post('/webhooks/stripe', express.raw({ type: 'application/json' }), async (req, res) => {
    try {
        ...
        if (event.type === 'payment_intent.succeeded') {
            ...
            await mongoService.updatePaymentStatus(
                paymentIntent.id,
                'failed',
...
router.get('/payments/history', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
...
router.get('/user/invoices', authMiddleware, async (req: any, res) => {
    try {
        const userId = req.user.sub;
        const invoices = await mongoService.getUserInvoices(userId);
        res.json({ invoices });
...
router.post('/public-maps', authMiddleware, async (req: any, res) => {
    try {
...
router.get('/public-maps/:publicId', async (req: any, res) => {
    try {
        ...
        if (fullDataset && fullDataset.data) {
            ...
            const graphSnapshot = fullDataset.data.find((r: any) => r.recordType === 'graph_snapshot');
            if (graphSnapshot && graphSnapshot.data) {
                const gapHandles = (jobOrchestrator as any).identifyEnrichmentGaps(graphSnapshot.data);
                if (gapHandles.length > 0) {
                    const isEnriching = associatedJobs.some((j: any) => j.metadata?.isEnriching === true);

                    if (!isEnriching) {
                        console.log(`[PublicRuntimeFill] ⚠️ Public Map ${publicId} has gaps. Triggering background task...`);
                        const mainJob = associatedJobs.find((j: any) => j.type === 'map_generation' || j.type === 'enrichment');

                        if (mainJob) {
                            const profileMap = new Map();
                            (jobOrchestrator as any).performDeepEnrichment(graphSnapshot.data, dataset.id, mainJob.id, profileMap).catch((err: any) => {
                                console.error(`[PublicRuntimeFill] ❌ Failed to enrich public map ${publicId}:`, err);
                            });
                            // Mark optimistically for the response below
                            (mainJob as any).metadata = { ...(mainJob as any).metadata, isEnriching: true };
                        }
                    }
                }
            }
        ...
        const hasEnrichingJob = associatedJobs.some((j: any) => j.metadata?.isEnriching === true);

        (fullDataset as any).isEnriching = hasEnrichingJob;
        res.json({ success: true, data: fullDataset, isEnriching: (fullDataset as any).isEnriching });
...
router.post('/plan-query', authMiddleware, approvalMiddleware, async (req: any, res) => {
    try {
...
router.post('/orchestration', authMiddleware, approvalMiddleware, async (req: any, res) => {
    try {
