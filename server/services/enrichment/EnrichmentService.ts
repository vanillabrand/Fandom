...
export const proxyMediaUrl = (url: string | undefined): string => {
    if (!url) return '';
    if (url.startsWith('data:')) return url; // Already base64
    if (url.includes('/api/proxy/image')) return url; // Already proxied
...
export class EnrichmentService {
     ...
     */
    public static enrichNode(node: any, profile: StandardizedProfile): void {
        ...
        if (existingEvidence) node.data.evidence = existingEvidence;
        if (existingProvenance) node.data.provenance = existingProvenance;
        if (existingCitation) node.data.citation = existingCitation;
        ...
        flatSyncFields.forEach(field => {
            if (node.data[field] !== undefined) {
                (node as any)[field] = node.data[field];
            }
        });
     ...
     */
    public static findProfileForNode(node: any, profileMap: Map<string, StandardizedProfile>): StandardizedProfile | undefined {
     ...
     */
    public static enrichGraph(analytics: any, profileMap: Map<string, StandardizedProfile>): number {
        if (!analytics || !analytics.root) return 0;
