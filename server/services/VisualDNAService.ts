...
export class VisualDNAService {
     ...
     * @param batchSize Number of images to analyze per cluster (default 10)
     */
    public static async enrichClustersWithVisuals(nodes: Node[], links: Link[], batchSize: number = 10): Promise<void> {
        ...
        const clusterToCreators = new Map<string, Node[]>();

        links.forEach(link => {
            const sourceId = (link.source && typeof link.source === 'object') ? (link.source as any).id : link.source;
            const targetId = (link.target && typeof link.target === 'object') ? (link.target as any).id : link.target;
            ...
            if (sourceNode?.group === 'cluster' && targetNode?.group === 'creator') {
                if (!clusterToCreators.has(sourceId)) clusterToCreators.set(sourceId, []);
                clusterToCreators.get(sourceId)!.push(targetNode);
            } else if (targetNode?.group === 'cluster' && sourceNode?.group === 'creator') {
                if (!clusterToCreators.has(targetId)) clusterToCreators.set(targetId, []);
