...
export class InvoiceService {
    private mailjetClient: any = null;
    ...
    private getMailjetClient() {
        if (!this.mailjetClient) {
            ...
            this.mailjetClient = new (Mailjet as any)(apiKey, secretKey);
        }
   ...
   */
    async generateMonthlyInvoice(userId: string, month: string): Promise<any> {
        ...
        const usageBreakdown: Record<string, { count: number; total: number; items: any[] }> = {};
        ...
        const invoice = {
            ...
            status: 'pending' as const,  // Changed from 'sent' to 'pending'
     ...
     */
    async generateAllMonthlyInvoices(month: string): Promise<{
        success: number;
        failed: number;
        results: any[];
    }> {
     ...
     */
    async sendInvoiceEmail(userId: string, invoiceId: string): Promise<boolean> {
        try {
            // Get invoice
     ...
     */
    private generateInvoiceEmailHTML(invoice: any, user: any): string {
...
<body>
     ...
     */
    private generateInvoiceEmailText(invoice: any, user: any): string {
        const monthYear = new Date(invoice.createdAt).toLocaleDateString('en-GB', {
            month: 'long',
            year: 'numeric'
        });
