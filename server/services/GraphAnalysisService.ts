...
export class GraphAnalysisService {
     ...
     */
    public static calculatePageRank(nodes: any[], links: any[], dampingFactor = 0.85, iterations = 20): Map<string, number> {
        console.log("[GraphAnalysis] Calculating PageRank...");

        const ranks = new Map<string, number>();
        const numNodes = nodes.length;
        if (numNodes === 0) return ranks;
        ...
        const incoming = new Map<string, string[]>(); // Map<TargetId, SourceIds[]>
     ...
     */
    public static applyInfluenceSizing(nodes: any[], ranks: Map<string, number>, baseSize = 5, multiplier = 1000): void {
        nodes.forEach(node => {
            ...
            if (node.group === 'cluster') newSize = Math.min(newSize, 60);
            else newSize = Math.min(newSize, 40); // Standard nodes capped at 40
