...
async function verifyDeepEnrichmentTrigger() {
    ...
    const gaps = jobOrchestrator.identifyEnrichmentGaps(mockAnalytics);
    ...
    jobOrchestrator.runApifyActor = async (actorId, input) => {
    ...
    mongoService.updateDataset = async (id, updates) => {
        console.log(`[TEST MOCK] updateDataset called for ${id}`, updates);
        return true;
    };
    mongoService.updateGraphSnapshot = async (id, data) => {
