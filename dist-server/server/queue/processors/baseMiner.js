...
class GeminiCache {
    ...
    generateKey(query, contextSize, platform = 'instagram') {
...
export class BaseMiner {
     ...
     */
    static async completeSubTask(jobId, resultKey, data) {
        ...
        await db.collection('jobs').updateOne({ id: jobId }, { $set: updateQuery });
        // 2. Check Aggregation Status
        await this.checkAggregation(jobId);
    }
    static async updateSubTaskProgress(jobId, resultKey, percent, stage) {
        const db = mongoService.getDb();
        // 1. Update individual sub-task status
        await db.collection('jobs').updateOne({ id: jobId }, {
            $set: {
        ...
        if (job && job.result && job.result.status) {
            ...
            const activeStages = [];
            if (status.structure === 'running')
                activeStages.push(`${icons.structure} Structure: ${status.structure_stage || 'Working'}`);
            if (status.trends === 'running')
                activeStages.push(`${icons.trends} Trends: ${status.trends_stage || 'Working'}`);
            if (status.creators === 'running')
                activeStages.push(`${icons.creators} Creators: ${status.creators_stage || 'Working'}`);
    ...
    static async checkAggregation(jobId) {
        const job = await mongoService.getJob(jobId);
        if (!job)
            return;
        ...
        if (scrapingDone) {
            ...
            try {
                ...
                if (results.structure && Array.isArray(results.structure.connections)) {
                    results.structure.connections.forEach((conn) => {
                ...
                allItems.forEach(item => {
                    switch (item.type) {
                        ...
                        case 'trend_topic':
                            ...
                            break;
                        case 'trend_hashtags':
                            const tagString = item.data.map((h) => `#${h.tag}`).join(' ');
                            context.push({
                ...
                if (cachedResult) {
                    ...
                    analysisResult = await analyzeFandomDeepDive(query, context, 'general_map', // Default intent for composite
                    'instagram', '', // No single dataset URL
                    100, // Scale
                    false, // themed
                        ...
                        if (cluster.children) {
                            cluster.children.forEach((node) => {
                                ...
                                if (node.type === 'creator')
                                    flatAnalytics.creators.push(item);
                                else if (node.type === 'brand')
                                    flatAnalytics.brands.push(item);
                                else if (node.type === 'topic')
                                    flatAnalytics.topics.push(item);
                            });
                        }
                    });
     ...
     */
    static convertTreeToGraph(treeData, visualData = null) {
        if (!treeData || !treeData.root)
            return { nodes: [], links: [] };
        const clusters = treeData.root.children || [];
        // [OPTIMIZATION] Pre-calculate array sizes to avoid resizing
        const totalLeaves = clusters.reduce((sum, cluster) => sum + (cluster.children?.length || 0), 0);
        ...
        // Pre-allocate arrays
        const nodes = new Array(estimatedNodes);
        const links = new Array(estimatedLinks);
        let nodeIdx = 0;
        let linkIdx = 0;
        // 1. Root
        const rootId = 'root';
        ...
        if (visualData && visualData.brands && Array.isArray(visualData.brands)) {
            visualData.brands.forEach((brand) => {
    ...
    static async handleFailure(jobId, outputKey, error) {
