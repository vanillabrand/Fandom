...
export class CostCalculator {
    constructor() {
        ...
        scraperRegistryRaw.forEach(scraper => {
     ...
     */
    async calculateStepPrice(actorId, recordCount) {
        ...
        if (!this.scraperMap.has(actorId)) {
            ...
            if (actorId.includes('followers'))
                baseRate = 5.00;
            else if (actorId.includes('profile'))
                baseRate = 4.60;
        }
     ...
     */
    async calculateQueryBuilderCost(profileCount = 50, actorId = 'apify/instagram-scraper') {
        await this.ensureConfig();
        // Load specific rate
     ...
     */
    async calculateQuickMapCost(profileCount = 20) {
        await this.ensureConfig();
     ...
     */
    async calculateDeepSearchCost(searchResults = 100) {
        await this.ensureConfig();
     ...
     */
    async calculateAiAnalysisCost(sampleSize = 100) {
        await this.ensureConfig();
     ...
     */
    async trackUsageAndDeduct(userId, action, description, cost) {
        // [FIX] Admin Bypass
