 ...
 */
export const generateDashboardConfig = (query, intent, analytics = null) => {
    const widgets = [];
    ...
    if (needsOverindexing) {
        ...
        const topBrands = (analytics?.overindexing?.topBrands || analytics?.brands || []).map((b) => ({ ...b, type: 'brand' }));
        const topCreators = (analytics?.overindexing?.topCreators || analytics?.creators || []).map((c) => ({ ...c, type: 'creator' }));
        const topTopics = (analytics?.topics || []).map((t) => ({ ...t, type: 'topic' }));
        const topHashtags = (analytics?.hashtags || []).map((h) => ({ ...h, type: 'hashtag' }));
    ...
    if ((intent === 'comparison' || intent === 'audience_overlap') && analytics?.comparison) {
        ...
        if (comp.profiles && Array.isArray(comp.profiles) && comp.profiles.length > 0 && comp.shared) {
            ...
            comp.profiles.forEach((profile, idx) => {
                widgets.push({
            ...
            if ((comp.shared.topics && comp.shared.topics.length > 0) || (comp.shared.brands && comp.shared.brands.length > 0)) {
                widgets.push({
                    ...
                    data: {
                        items: [
                            ...(comp.shared.topics || []).map((topic) => ({
                                id: topic,
                                title: topic,
                                type: 'topic'
                            })),
                            ...(comp.shared.brands || []).map((brand) => ({
                                id: brand,
                                title: brand,
                                type: 'brand'
                            }))
    ...
    if (intent === 'hashtag_tracking') {
        ...
        widgets.push({
            ...
            rowSpan: 6,
            data: {
                items: (analytics?.creators || []).map((c) => ({
                    id: c.handle,
    ...
    if (intent === 'ugc_discovery') {
        ...
        widgets.push({
            ...
            rowSpan: 6,
            data: {
                items: (analytics?.overindexing?.topCreators || []).map((c) => ({
                    id: c.handle,
                    title: c.name || c.handle,
                    description: c.citation
                }))
    ...
    if (intent === 'sentiment_analysis') {
        ...
        if (sentimentData?.top_themes && sentimentData.top_themes.length > 0) {
            widgets.push({
                ...
                rowSpan: 4,
                data: {
                    items: sentimentData.top_themes.map((t) => ({
                        id: t.term,
                        title: t.term,
    ...
    if (intent === 'geo_discovery') {
        ...
        const sortedGeo = geoData.sort((a, b) => b.count - a.count).slice(0, 10);
        widgets.push({
            id: 'geo_chart',
            type: 'ChartPanel',
            ...
            rowSpan: 6,
            data: {
                labels: sortedGeo.map((d) => d.name),
                datasets: [{
                        label: 'Fan Density',
                        data: sortedGeo.map((d) => d.count),
