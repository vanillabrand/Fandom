...
class EmailService {
    ...
    async sendViaMailjet(to, subject, htmlBody, textBody) {
        const payload = {
            Messages: [
                {
                    From: {
    ...
    async sendTitleAlert(title, errorDetails) {
...
`;
        fs.appendFile(this.logPath, logEntry, (err) => {
            if (err)
                console.error('[EmailService] Failed to write to log file:', err);
            else
                console.log('[EmailService] Email logged to file.');
        });
    }
}
