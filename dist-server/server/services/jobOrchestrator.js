        ...
        try {
            ...
            if (record.metaData) {
                ...
                if (record.latestPosts && Array.isArray(record.latestPosts)) {
                    standard.latestPosts = record.latestPosts.map((p) => ({
                        ...
                        children: (p.images || p.childPosts || []).map((c) => ({
                            id: c.id,
            ...
            // 2. Instagram Profile Scraper (Deep Dive - 'owner' object or flat)
                ...
                if (record.edge_owner_to_timeline_media && record.edge_owner_to_timeline_media.edges) {
                    standard.latestPosts = record.edge_owner_to_timeline_media.edges.map((e) => {
                        const node = e.node;
                        return {
                            id: node.id,
                            ...
                            children: node.edge_sidecar_to_children?.edges?.map((edge) => ({
                                id: edge.node.id,
    ...
    async enrichFandomAnalysisParallel(analytics, profileMap) {
        ...
        const collectNodes = (node) => {
            if (!node || visited.has(node))
                return; // Skip null nodes and already visited nodes
            visited.add(node);
            allNodes.push(node);
            ...
            if (analysis) {
                ...
                if (analysis && analysis.root && analysis.root.children) {
                    ...
                    analytics.creators = allNodes
                        ...
                        .sort((a, b) => {
                        const scoreA = (a.overindexScore || a.affinityPercent || a.affinity || 0);
                        const scoreB = (b.overindexScore || b.affinityPercent || b.affinity || 0);
                        if (scoreB !== scoreA)
                            return scoreB - scoreA;
                        const getVal = (v) => {
                            if (typeof v === 'number')
        ...
        records.forEach(r => {
            ...
            const getOrCreate = () => {
                ...
                if (!p) {
                    ...
                    if (pk)
                        profileMap.set(pk, p); // [CRITICAL] Index by ID
                }
                return p;
            };
        ...
        if (analytics && analytics.root) {
            ...
            sortedOverindexed.forEach(([username, frequency]) => {
                ...
                if (hydrated.group === 'brand')
                    extractedAnalytics.brands.push(hydrated.data);
                else
                    extractedAnalytics.creators.push(hydrated.data);
