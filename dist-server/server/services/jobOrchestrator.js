...
import { analyzeFandomDeepDive, aggregateLocations } from '../../services/geminiService.js';
...
export class JobOrchestrator {
     ...
     */
    async enrichFandomAnalysisParallel(analytics, profileMap) {
        if (!analytics)
            return analytics;
    ...
    finalizeAnalytics(analytics) {
        ...
        const deduplicate = (list, idKey = 'username') => {
            if (!list || !Array.isArray(list))
                return [];
            const seen = new Set();
            return list.filter(item => {
                ...
                if (!val || val === 'unknown' || seen.has(val))
                    return false;
                seen.add(val);
                return true;
            });
        ...
        if (target.creators)
            target.creators = deduplicate(target.creators);
        if (target.brands)
            target.brands = deduplicate(target.brands, 'name'); // [FIX] Use 'name' for brands
        if (target.topics)
            target.topics = deduplicate(target.topics, 'name');
        if (target.subtopics)
            target.subtopics = deduplicate(target.subtopics, 'name');
        if (target.clusters)
            target.clusters = deduplicate(target.clusters, 'name');
        ...
        if (target.creators)
            target.creators.sort(sortingFn);
        if (target.brands)
            target.brands.sort(sortingFn);
        return analytics;
    }
    /**
     * Identifies nodes in the graph that lack enriched data (metrics, bio, etc.)
     */
    identifyEnrichmentGaps(analytics) {
        if (!analytics)
            return [];
        const gaps = new Set();
        const visited = new Set();
        ...
        const targetAnalytics = analytics.analytics || analytics;
        if (targetAnalytics.creators && Array.isArray(targetAnalytics.creators))
            targetAnalytics.creators.forEach(node => checkNode(node));
        if (targetAnalytics.brands && Array.isArray(targetAnalytics.brands))
            targetAnalytics.brands.forEach(node => checkNode(node));
        if (targetAnalytics.overindexing?.topCreators && Array.isArray(targetAnalytics.overindexing.topCreators)) {
            targetAnalytics.overindexing.topCreators.forEach((node) => checkNode(node));
        }
        if (targetAnalytics.topContent && Array.isArray(targetAnalytics.topContent)) {
            targetAnalytics.topContent.forEach((item) => {
     ...
     */
    async performDeepEnrichment(analytics, datasetId, jobId, profileMap) {
        ...
        try {
            ...
            if (stepResult && stepResult.items) {
                ...
                for (const item of stepResult.items) {
                    ...
                    if (profile) {
                        ...
                        if (!existing && pk)
                            existing = profileMap.get(pk) || profileMap.get(String(pk));
                        if (existing) {
                            ...
                            indexProfile(`@${cleanHandle}`, existing);
                            if (pk)
                                indexProfile(String(pk), existing);
                            if (slugifiedName && slugifiedName.length > 2)
                                indexProfile(slugifiedName, existing);
                        }
                        else {
                            // Fresh entry
                            ...
                            indexProfile(`@${cleanHandle}`, profile);
                            if (pk)
                                indexProfile(String(pk), profile);
                            if (slugifiedName && slugifiedName.length > 2)
                                indexProfile(slugifiedName, profile);
                        }
                ...
                if (datasetId) {
                    ...
                    try {
                        ...
                        await mongoService.getDb().collection('records').updateOne({ datasetId, recordType: 'analytics_data' }, {
                            $set: {
                ...
                if (datasetId) {
                    await mongoService.updateDataset(datasetId, { 'isEnriching': false });
                }
                ...
                if (jobId) {
                    await mongoService.updateJob(jobId, { 'metadata.isEnriching': false });
                }
                if (datasetId) {
                    await mongoService.updateDataset(datasetId, { 'isEnriching': false });
                }
            ...
            if (jobId) {
                await mongoService.updateJob(jobId, { 'metadata.isEnriching': false }).catch(() => { });
            }
            if (datasetId) {
     ...
     */
    getEnrichmentConfig(totalNodes) {
    ...
    async startPolling(intervalMs = 10000) {
        ...
        try {
            if (mongoService.isConnected()) {
                ...
                const result = await db.collection('jobs').updateMany({ status: 'running' }, { $set: { status: 'queued', updatedAt: new Date() } });
    ...
    async pollNextJob() {
        ...
        try {
            ...
            const result = await db.collection('jobs').findOneAndUpdate({ status: 'queued' }, { $set: { status: 'running', updatedAt: new Date() } }, { sort: { createdAt: 1 }, returnDocument: 'after' } // Ensure we get the updated job
            );
     ...
     */
    async runSeedScrape(query, jobId, ignoreCache = false) {
        // 1. Extract handles
        const handles = [];
    ...
    async processMapGeneration(job) {
        ...
        const results = [];
        let datasetId = "";
        const datasetIds = []; // [NEW] Track all dataset IDs
        ...
        for (let i = 0; i < plan.steps.length; i++) {
            ...
            try {
                ...
                if (step.actorId.includes('followers') || step.actorId.includes('following') || step.actorId === 'asIjo32NQuUHP4Fnc') {
                    ...
                    items = items.filter((item) => !item.isPrivate && !item.is_private);
        ...
        if (results.length > 0) {
            ...
            try {
                ...
                const intent = plan.intent || 'general_map';
                ...
                if (intent === 'competitor_content_analysis') {
                    ...
                    analysisResult = await analyzeFandomDeepDive(query, allItems, // [FIX] Pass Data Array (Type: any[])
                    intent, 'instagram', datasetUrl, sampleSize || 100, // Pass scaling parameter
                ...
                if (analysisResult && (analysisResult.root || analysisResult.graph)) {
                    ...
                    try {
                        ...
                        if (analysisResult.graph && analysisResult.graph.nodes) {
                            const enrichedGraph = await this.enrichFandomAnalysisParallel({ root: { children: analysisResult.graph.nodes } }, profileMap);
                ...
                if (analysisResult && analysisResult.graph && analysisResult.graph.nodes) {
                    ...
                    try {
                        const hierarchicalGraph = this.buildGraphHierarchy(analysisResult.graph.nodes, analysisResult.graph.links || []);
                ...
                if (analysisResult && analysisResult.root) {
                    ...
                    const flatAnalytics = {
                        ...
                        clusters: [],
                        topics: [],
                        subtopics: [], // [FIX] Add subtopics
                        overindexing: { topCreators: [] }, // [FIX] Add overindexing
                        nonRelatedInterests: [],
                        topContent: [],
                            ...
                            cluster.children.forEach((node) => {
                                ...
                                if (node.type === 'creator') {
                                    ...
                                    if (!topicsMap.has(key)) {
                                        ...
                                        const parsePct = (s) => parseInt(s?.replace('%', '') || '0');
                    ...
                    clusters.forEach((c) => {
                        if (c.data && c.data.latestPosts && Array.isArray(c.data.latestPosts)) {
                            c.data.latestPosts.forEach((post) => {
                                const pid = post.id || post.shortCode || post.url;
                                if (pid && !uniquePosts.has(pid)) {
                    ...
                    flatAnalytics.topContent = Array.from(uniquePosts.values())
                        .sort((a, b) => (b.likesCount || 0) - (a.likesCount || 0)) // Sort by Likes
                        .slice(0, 12) // Top 12
                        .map(post => ({
        ...
        try {
            ...
            await costCalculator.trackUsageAndDeduct(job.userId, 'query_builder', `Map Generation: ${query}`, cost);
    ...
    // --- HELPER METHODS ---
    resolveInput(input, previousResults, plan, actorId) {
        ...
        const extractUsernames = (items) => {
            ...
            if (actorId && (actorId.includes('comment-scraper') || actorId.includes('media-scraper'))) {
                ...
                const isValidPostUrl = (u) => u && (u.includes('/p/') || u.includes('/reel/'));
                (items || []).forEach((item) => {
                    // 1. Direct Post URL
                    if (isValidPostUrl(item.url))
                        urls.push(item.url);
                    else if (isValidPostUrl(item.postUrl))
                        urls.push(item.postUrl);
                        ...
                        if (typeof item === 'string' && item.includes('USE_DATA_FROM')) {
                            ...
                            if (item.includes('USE_DATA_FROM_STEP_')) {
                                ...
                                if (plan && plan.steps) {
                                    ...
                                    const stepIndex = plan.steps.findIndex((s) => (s.id === stepId || s.stepId === stepId));
                                    if (stepIndex >= 0 && previousResults[stepIndex]) {
                                        sourceItems = previousResults[stepIndex];
                                    }
                                    else {
                                        console.warn(`[JobOrchestrator] âš ï¸ Could not resolve step '${stepId}'. Available steps:`, plan.steps.map((s) => s.id || s.stepId));
                                    }
                                }
                            }
                            else {
        ...
        if (newInput.search && Array.isArray(newInput.search)) {
            ...
            if (newInput.search.length === 1) {
                ...
                if (!newInput.directUrls) {
                    newInput.directUrls = newInput.search.map((u) => u.startsWith('http') ? u : `https://www.instagram.com/${u.replace('@', '')}`);
                }
     ...
     */
    processGoogleSearchResults(googleResults, coreProfile) {
     ...
     */
    highlightReferencePosts(enrichedProfile, coreProfile, searchKeywords) {
        if (!enrichedProfile.latestPosts || !Array.isArray(enrichedProfile.latestPosts)) {
            return enrichedProfile;
        }
        enrichedProfile.latestPosts = enrichedProfile.latestPosts.map((post) => {
        ...
        const relevantPostCount = enrichedProfile.latestPosts.filter((p) => p.isRelevantPost).length;
     ...
     */
    async auditMinerResults(query, minerOutput, originalProfiles) {
        const issues = [];
        const suggestions = [];
        // Check cluster count
        ...
        minerOutput.clusters?.forEach((c) => {
            c.members?.forEach((m) => clusteredProfiles.add(m.username || m.id));
        });
        ...
        if (hasLocationQuery) {
            const hasLocationCluster = minerOutput.clusters?.some((c) => /\b(location|based|london|new york|paris|tokyo|city)\b/i.test(c.name || ''));
            if (!hasLocationCluster) {
                suggestions.push('Query mentions location but no location-based clusters found.');
            }
        }
        if (hasEngagementQuery) {
            const hasEngagementMetrics = minerOutput.clusters?.some((c) => c.avgEngagementRate || (c.description || '').includes('engagement'));
    ...
    analyzeNetworkFrequency(results, intent = 'general_map') {
    ...
    aggregateContextLocal(results, intent) {
        ...
        if (intent === 'over_indexing' || intent === 'network_clusters' || intent === 'influencer_identification') {
            ...
            if (overindexed.length > 0) {
                ...
                overindexed.forEach((p, idx) => {
                    ...
                    const item = flatResults.find((i) => (i.username || i.ownerUsername)?.toLowerCase() === p.username.toLowerCase());
                    if (item)
                        item._frequency = p.frequency;
                });
                context += `\n--- END OVER-INDEXED ---\n\n`;
            }
        }
        // 1. Comments (High Value for Sentiment)
        const comments = flatResults.filter((i) => i.text && i.ownerUsername);
        if (comments.length > 0) {
            context += `--- USER COMMENTS (${comments.length}) ---\n`;
            comments.slice(0, 50).forEach((c) => {
                context += `[User: ${c.ownerUsername}] "${c.text}" (Likes: ${c.likesCount || 0})\n`;
            });
            context += `\n`;
        }
        // 2. Posts (Captions)
        const posts = flatResults.filter((i) => i.caption && !i.text); // Exclude comments which might have caption field
        if (posts.length > 0) {
    ...
    async runApifyActor(actorId, input, jobId, metadata) {
        ...
        if (tokens.length === 0)
            throw new Error("Apify Token missing (APIFY_API_TOKENS or APIFY_TOKEN)");
        ...
        if (realActorId.includes('~'))
            realActorId = realActorId.replace(/~/g, '/');
        const actorMapping = {
        ...
        if (realActorId === 'asIjo32NQuUHP4Fnc' || realActorId === 'IkdNTeZnRfvDp8V25') {
            ...
            if (normalizedInput.username && Array.isArray(normalizedInput.username)) {
                ...
                normalizedInput.username = normalizedInput.username.map((u) => {
        ...
        if (realActorId.includes('instagram-api-scraper') && (normalizedInput.usernames || normalizedInput.directUrls)) {
            ...
            normalizedInput.directUrls = uniqueTargets.map((u) => {
                const clean = u.replace('@', '').trim();
            ...
            if (Array.isArray(normalizedInput.directUrls) && normalizedInput.directUrls.length > 1) {
                ...
                if (!normalizedInput.resultsLimit)
                    normalizedInput.resultsLimit = perUserLimit;
            }
            ...
            delete normalizedInput.usernames;
            if (!normalizedInput.resultsType)
                normalizedInput.resultsType = 'posts';
            ...
            if (normalizedInput.search) {
                ...
                if (!normalizedInput.resultsLimit)
                    normalizedInput.resultsLimit = 6;
        ...
        try {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                if (i > 0)
                    console.log(`[Apify] ðŸ”„ Rotating to Token #${i + 1} for execution...`);
            ...
            if (realActorId === 'apify/google-search-scraper' || realActorId.includes('google-search')) {
                ...
                const organicResults = items.flatMap((item) => item.organicResults || []);
            ...
            const hasRestrictedContent = items.some((item) => {
                ...
                if (bio.includes('Restricted profile'))
                    return true;
    ...
    async syncApifyDatasetToLocal(apifyDatasetId, name, userId, aiAnalytics = null, allResults = [], profileMap) {
        const apifyToken = process.env.APIFY_API_TOKEN || process.env.APIFY_TOKEN;
        ...
        if (items.length > 0 || (aiAnalytics && aiAnalytics.root)) {
            ...
            if (aiAnalytics && aiAnalytics.graph && aiAnalytics.graph.nodes && aiAnalytics.graph.links && aiAnalytics.graph.links.length > 0) {
                ...
                // 1. Add Nodes
                aiAnalytics.graph.nodes.forEach((n) => {
                ...
                const traverseAndHydrate = (node, parentId) => {
                    ...
                    const getMatchedData = (n) => {
                        ...
                        for (const raw of rawCandidates) {
                            ...
                            if (masterProfileMap.has(clean))
                                return { cleanId: clean, data: masterProfileMap.get(clean) };
                        }
                    ...
                    if (rawType !== 'root' && rawType !== 'main' && nodeId !== 'MAIN') {
                        ...
                        const richData = {
                            ...
                            latestPosts: (Array.isArray(realPosts) && realPosts.length > 0)
                                ? realPosts.map((p) => proxyMediaFields(p)).filter(Boolean)
                        ...
                        addNode({
                            ...
                            color: this.getNodeColor ? this.getNodeColor(group) : '#94a3b8',
                            data: richData
                        });
                        if (parentId) {
                            links.push({
                    ...
                    if (node.children && Array.isArray(node.children)) {
                        ...
                        node.children.forEach((child) => traverseAndHydrate(child, nextParentId));
                    }
                };
                ...
                items.forEach((item, idx) => {
                    ...
                    addNode({
                        ...
                        data: {
                            ...
                            latestPosts: (Array.isArray(item.latestPosts) ? item.latestPosts : []).map((p) => proxyMediaFields(p)).filter(Boolean)
                        }
                    });
            ...
            if (aiAnalytics && (aiAnalytics.graph || aiAnalytics.root)) {
                if (items && Array.isArray(items) && items.length > 0) {
                    ...
                    items.forEach((item, idx) => {
                        ...
                        if (idx < 5)
                            console.log(`[Backfill Debug] Checking ${cleanId} (Raw: ${rawHandle}). Exists? ${addedNodeIds.has(cleanId)}`);
     ...
     */
    auditAndRepairPlan(plan, query, sampleSize, postLimit = 3) {
            ...
            if (step.input) {
                ...
                if (index === 0) {
                    ...
                    if (isPostScrape) {
                        ...
                        if (step.input.resultsLimit)
                            step.input.resultsLimit = finalLimit;
                        if (step.input.maxPosts)
                            step.input.maxPosts = finalLimit;
                ...
                if (step.input.searchLimit)
                    step.input.searchLimit = Math.min(effectiveLimit, step.input.searchLimit);
            }
        });
        return plan;
    }
    ...
    async processAiAnalysis(job) {
        ...
        let analysisResult = null;
        try {
            analysisResult = await analyzeFandomDeepDive(query, [], // No scraped context for pure AI analysis
            job.metadata.intent || 'general_map', platform || 'instagram', '', sampleSize || 100, // [NEW] Pass scaling parameter
        ...
        if (analysisResult) {
            ...
            const traverse = (node) => {
                ...
                if (node.data?.handle)
                    candidates.add(node.data.handle.replace('@', '').trim());
                if (node.data?.username)
                    candidates.add(node.data.username.replace('@', '').trim());
                ...
                for (let i = 0; i < handlesToScrape.length; i += BATCH_SIZE) {
                    ...
                    try {
                        ...
                        if (scrapeResult && scrapeResult.items) {
                            const batchProfiles = scrapeResult.items.filter((p) => p && p.username);
            ...
            for (const item of scrapedProfiles) {
                ...
                if (profile) {
                    if (profile.id)
                        profileMap.set(profile.id, profile);
                    if (profile.username)
                        profileMap.set(profile.username.toLowerCase().replace('@', '').trim(), profile);
                }
            }
            const localId = uuidv4();
            ...
            try {
                ...
                await costCalculator.trackUsageAndDeduct(job.userId, 'quick_map', // Mapped to quick_map action for now
                `AI Analysis: ${query}`, cost);
    ...
    async processOrchestration(job) {
        ...
        if (['over_indexing', 'brand_affinity', 'network_clusters'].includes(plan.intent)) {
            ...
            itemsToAnalyze = Array.from(uniqueMap.values()).sort((a, b) => {
            ...
            itemsToAnalyze = results.flat().sort((a, b) => {
        ...
        itemsToAnalyze.forEach(rawItem => {
            ...
            if (!standard)
                return; // Skip invalid/private profiles without crashing
            ...
            if (finalUsername) {
                ...
                if (!existing) {
                    ...
                    if (!existing.fullName && standard.fullName)
                        existing.fullName = standard.fullName;
                    if (!existing.biography && standard.biography)
                        existing.biography = standard.biography;
                    if (!existing.profilePicUrl && standard.profilePicUrl)
                        existing.profilePicUrl = standard.profilePicUrl;
                    if (!existing.externalUrl && standard.externalUrl)
                        existing.externalUrl = standard.externalUrl;
                    // Max numeric fields
                    if (standard.followersCount)
                        existing.followersCount = Math.max(existing.followersCount || 0, standard.followersCount);
                    if (standard.followsCount)
                        existing.followsCount = Math.max(existing.followsCount || 0, standard.followsCount);
                    // Merge Posts
                    if (standard.latestPosts && standard.latestPosts.length > 0) {
                        if (!existing.latestPosts)
                            existing.latestPosts = [];
                        // Dedupe posts by ID
                        const pidSet = new Set(existing.latestPosts.map((p) => p.id));
                        standard.latestPosts.forEach(p => {
                            if (!pidSet.has(p.id))
                                existing.latestPosts.push(p);
                        });
                    }
                }
            }
        ...
        if (enrichedProfiles.length > 0) {
            const sampleUsernames = enrichedProfiles.slice(0, 3).map((p) => p.username).join(', ');
        ...
        if (['bio_search', 'influencer_identification'].includes(plan.intent)) {
            ...
            if (keyword) {
                ...
                const relevantItems = itemsToAnalyze.filter((p) => (p.biography || p.bio || '').toLowerCase().includes(keyword) ||
        ...
        try {
            ...
            const analysis = await analyzeFandomDeepDive(query, itemsToAnalyze, plan.intent, plan.platform || 'instagram', datasetUrl, sampleSize, // Pass scaling parameter
            ...
            if (analysis) {
                ...
                const potentialHandles = new Set();
                const harvestHandles = (node) => {
                    if (!node)
                        return;
                    if (node.handle)
                        potentialHandles.add(node.handle);
                    if (node.data?.handle)
                        potentialHandles.add(node.data.handle);
                    if (node.children)
                        node.children.forEach(harvestHandles);
                };
                harvestHandles(analysis.root);
                if (analysis.creators)
                    analysis.creators.forEach((c) => potentialHandles.add(c.handle));
                if (analysis.brands)
                    analysis.brands.forEach((b) => potentialHandles.add(b.handle));
                ...
                if (analytics.subtopics && Array.isArray(analytics.subtopics)) {
                    analytics.subtopics.forEach((sub) => {
                        // Advanced Matching Strategy: Score-based
                        const keywords = (sub.keywords || []).map((k) => k.toLowerCase());
                ...
                if (segments && Array.isArray(segments)) {
                    ...
                    segments.forEach((seg, idx) => {
                        ...
                        if (seg.brands_creators && Array.isArray(seg.brands_creators)) {
                            seg.brands_creators.forEach((item) => {
                ...
                if (['bio_search', 'influencer_identification'].includes(plan.intent) && itemsToAnalyze.length > 0) {
                    const existingHandles = new Set((analytics.creators || []).map((c) => c.handle.toLowerCase()));
                    const hydratedProfiles = itemsToAnalyze.map((p) => {
                        ...
                        return {
                            ...
                            // Video/Media Data
                            latestPosts: (p.latestPosts || p.posts || []).slice(0, 3).map((post) => ({
                            ...
                            relatedProfiles: (p.relatedProfiles || []).map((rp) => ({
                    ...
                    if (hydratedProfiles.length > 0) {
                        ...
                        if (!analytics.overindexing)
                            analytics.overindexing = {};
                        if (!analytics.overindexing.topCreators)
                            analytics.overindexing.topCreators = analytics.creators;
                    }
                }
                ...
                if (analysis && analysis.root && analysis.root.children) {
                    ...
                    if (analysis.topContent || allNodes.some(n => n.type === 'post' || n.group === 'content')) {
                        ...
                        const contentFromAI = (analysis.topContent || []).map((item) => ({
        ...
        if (analytics && analytics.visualAnalysis) {
            analytics.visual = {
                ...
                aesthetics: (analytics.visualAnalysis.aestheticTags || []).map((t) => ({ style: t, score: 0.9 })),
            ...
            if ((!analytics.visual.brands || analytics.visual.brands.length === 0) && analytics.brands) {
                analytics.visual.brands = analytics.brands.map((b) => ({ name: b.name || b.label, count: b.count || 5 }));
            }
            // Map to Frontend visualTheme
            analytics.visualTheme = {
        ...
        if (graphData.analytics) {
            ...
            keys.forEach(key => {
                if (graphData.analytics[key] && Array.isArray(graphData.analytics[key])) {
                    ...
                    const existingMap = new Map(analytics[key].map((i) => [i.username || i.handle || i.name || i.label, i]));
                    graphData.analytics[key].forEach((item) => {
            ...
            if (job.id) {
                await mongoService.updateJob(job.id, { "metadata.isEnriching": true });
            }
            ...
            if (job.id) {
                await mongoService.updateJob(job.id, { "metadata.isEnriching": false });
            }
            // Merge scalar props
            ...
            if (analytics.brands)
                analytics.brands = dedupeList(analytics.brands);
            if (analytics.creators)
                analytics.creators = dedupeList(analytics.creators);
            ...
            if (analytics.brands && itemsToAnalyze.length > 0) {
                ...
                analytics.brands.forEach((brand) => {
            ...
            if (analytics.creators && itemsToAnalyze.length > 0) {
                ...
                analytics.creators.forEach((creator) => {
        ...
        const analyticsRecord = {
            datasetId: localId,
            recordType: 'analytics_data', // [FIX] Strict type
            platform: (plan.platform || 'instagram'),
        ...
        const snapshotRecord = {
            datasetId: localId,
            recordType: 'graph_snapshot', // [FIX] Strict type
            platform: (plan.platform || 'instagram'),
        ...
        try {
            ...
            if (totalChargedAmount > 0) {
                await costCalculator.trackUsageAndDeduct(job.userId, 'query_builder', `Orchestration: ${query}`, {
     ...
     */
    programmaticDatasetMatch(query, existingDatasets, requestedSample = 100, requestedDepth = 3) {
     ...
     */
    async analyzeMapRequirements(query, sampleSize = 100, existingDatasets = [], ignoreCache = false, useDeepAnalysis = false, seedContext = "", postLimit = 3) {
    ...
    **CRITICAL RULES:**
     ...
     */
    validateAndFixActorIds(plan, effectiveDatasets = []) {
        const ACTOR_REPLACEMENTS = {
            'datadoping/instagram-following-scraper': 'thenetaji/instagram-followers-followings-scraper',
            'datadoping/instagram-follower-scraper': 'thenetaji/instagram-followers-followings-scraper',
        ...
        if (plan.steps && Array.isArray(plan.steps)) {
            plan.steps.forEach((step) => {
                ...
                if (actorField && ACTOR_REPLACEMENTS[actorField]) {
                    ...
                    if (step.actorId)
                        step.actorId = ACTOR_REPLACEMENTS[actorField];
                    if (step.actor)
                        step.actor = ACTOR_REPLACEMENTS[actorField];
                }
                ...
                if (finalActor && (finalActor === 'apify/instagram-api-scraper' || finalActor.includes('instagram-api-scraper'))) {
                    if (step.input && step.input.usernames && !step.input.directUrls) {
                        ...
                        delete step.input.usernames;
                        // Ensure other props
                        if (!step.input.resultsType)
                            step.input.resultsType = 'details';
                        if (step.input.addParentData === undefined)
                            step.input.addParentData = true;
                    }
                }
            });
        }
        // [ENHANCED] Validate existingDatasetIds from AI (Hybrid Approach - Option C)
        let reusedDatasetDetails = {};
     ...
     */
    async recalculateStepCosts(plan) {
        if (!plan.steps || !Array.isArray(plan.steps))
            return plan;
        ...
        if (plan.reusedDatasetDetails) {
            totalCachedRecords = Object.values(plan.reusedDatasetDetails).reduce((acc, curr) => acc + (curr.recordCount || 0), 0);
        }
        for (let i = 0; i < plan.steps.length; i++) {
            const step = plan.steps[i];
            ...
            if (recordCount === undefined || recordCount === null || isNaN(recordCount)) {
                if (step.input && step.input.maxCount)
                    recordCount = step.input.maxCount;
                else if (step.input && step.input.limit)
                    recordCount = step.input.limit;
                else if (step.input && step.input.resultsLimit)
                    recordCount = step.input.resultsLimit;
                else if (step.input && step.input.searchLimit)
                    recordCount = step.input.searchLimit;
                else if (step.input && step.input.sampleSize)
                    recordCount = step.input.sampleSize;
                else
                    recordCount = 100; // Safe default
            }
     ...
     */
    async checkPlanForCachedSteps(plan) {
        if (!plan.steps || !Array.isArray(plan.steps))
            return plan;
        ...
        for (const step of plan.steps) {
            try {
                ...
                if (realActorId.includes('instagram-api-scraper') && (normalizedInput.usernames || normalizedInput.directUrls)) {
                    const rawTargets = normalizedInput.directUrls || normalizedInput.usernames;
                    const targets = Array.isArray(rawTargets) ? rawTargets : [rawTargets];
                    normalizedInput.directUrls = targets.map((u) => {
                        const clean = u.replace('@', '').trim();
                        return clean.startsWith('http') ? clean : `https://www.instagram.com/${clean}`;
                    });
                    delete normalizedInput.usernames;
     ...
     */
    injectUniversalEnrichment(planData, sampleSize) {
    ...
    async executeOrchestrationPlan(job, plan) {
        const results = [];
        const stepResultsMap = new Map(); // [FIX] Store step results by ID for precise resolution
        const datasetIds = []; // [NEW] Track dataset IDs
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            try {
                ...
                const runOutput = await this.runApifyActor(step.actorId, resolvedInput, job.id, {
                    ...
                    query: job.metadata?.query || plan.query || 'orchestration',
                    planId: job.id,
                ...
                if (runOutput.datasetId) {
                    if (!datasetIds.includes(runOutput.datasetId))
                        datasetIds.push(runOutput.datasetId);
                ...
                if (i === 0) {
                    stepResult = stepResult.map((item) => ({ ...item, isSourceAccount: true }));
                }
                ...
                if (filters.bioKeywords && filters.bioKeywords.length > 0 && stepResult.length > 0) {
                    ...
                    stepResult = stepResult.filter((item) => {
                        ...
                        if (!bio)
                            return true;
                        return filters.bioKeywords.some((kw) => bio.toLowerCase().includes(kw.toLowerCase()));
                    });
     ...
     */
    resolveOrchestrationInput(input, stepResults, plan, limit = 500) {
    ...
    extractDataFromStep(stepId, stepResults, limit) {
    ...
    aggregateProfiles(records) {
        ...
        records.forEach(r => {
            ...
            const getOrCreate = () => {
                ...
                if (!p && pk)
                    p = profileMap.get(pk); // Try ID lookup
                if (!p) {
                    p = {
                    ...
                    if (pk)
                        profileMap.set(String(pk), p); // [CRITICAL] Index by string ID
                }
                return p;
            };
            ...
            if (owner && owner.toLowerCase() !== cleanUser && (r.type === 'followings' || r.type === 'following' || r.query === 'followings')) {
                ...
                if (!ownerProfile.follows)
                    ownerProfile.follows = [];
                // Avoid redundant entries
                if (!ownerProfile.follows.find((f) => (f.username || f) === username)) {
            ...
            // 3. Contact Info
            if (sourceEmail && typeof sourceEmail === 'string' && !profile.email)
                profile.email = sourceEmail;
            if (sourceExternalUrl && typeof sourceExternalUrl === 'string' && !profile.externalUrl)
                profile.externalUrl = sourceExternalUrl;
            // [FIX] Verification Status
            if (r.isBusinessAccount !== undefined)
                profile.isBusinessAccount = r.isBusinessAccount;
            if (meta.isBusinessAccount !== undefined)
                profile.isBusinessAccount = meta.isBusinessAccount;
            if (r.isVerified !== undefined)
                profile.isVerified = r.isVerified;
            if (meta.verified !== undefined)
                profile.isVerified = meta.verified;
            if (ownerObj.is_verified !== undefined)
                profile.isVerified = ownerObj.is_verified;
            ...
            if (isPost) {
                ...
                if (!profile.latestPosts.find((p) => p.id === r.id || (p.shortCode && p.shortCode === r.shortCode))) {
            ...
            if (profile.latestPosts.length > 0) {
                const totalInteractions = profile.latestPosts.reduce((acc, post) => acc + (post.likesCount || 0) + (post.commentsCount || 0), 0);
                const totalLikes = profile.latestPosts.reduce((acc, post) => acc + (post.likesCount || 0), 0);
            ...
            if (r.latestPosts && Array.isArray(r.latestPosts)) {
                r.latestPosts.forEach((p) => {
                    if (!profile.latestPosts.find((existing) => existing.id === p.id)) {
    ...
    collectEvidence(p, query) {
        ...
        if (p.latestPosts && p.latestPosts.length > 0) {
            p.latestPosts.slice(0, 5).forEach((post) => {
        ...
        if (p.comments && Array.isArray(p.comments) && p.comments.length > 0) {
            ...
            const relevantComment = p.comments.find((c) => c.text && c.text.toLowerCase().includes(query.toLowerCase()));
            if (relevantComment) {
                evidence.push({
                    type: 'comment',
                    label: 'User Comment',
     ...
     */
    createEvidenceSubnodes(sourceNode, evidenceItems, nodes, links) {
        ...
        evidenceItems.forEach((ev, idx) => {
            ...
            if (ev.type === 'post')
                subGroup = 'evidence_post'; // Distinct but still 'evidence' prefix
            const subNode = {
                id: subNodeId,
    ...
    hydrateNodeData(p, group, evidence, query) {
        if (!p)
            return {};
     ...
     */
    generateGraphFromResults(plan, results, query, analytics = null) {
            ...
            if (plan.intent === 'over_indexing' || plan.intent === 'brand_affinity' || plan.intent === 'audience_overlap') {
                return this.generateOverindexGraph(richProfiles, query, plan, analytics).then(res => {
                    ...
                    if (res && plan && plan.intent) {
                        this.optimizeGraphTopology(res.nodes, res.links, plan.intent, res.nodes.find((n) => n.id === 'root' || n.id === 'MAIN') || { id: 'MAIN' }, query);
                    }
                    return res;
                });
            }
        ...
        if (graphResult && !(graphResult instanceof Promise) && plan && plan.intent) {
            this.optimizeGraphTopology(graphResult.nodes, graphResult.links, plan.intent, graphResult.nodes.find((n) => n.id === 'root' || n.id === 'MAIN') || { id: 'MAIN' }, query);
        }
     ...
     */
    generateTreeFromServerData(profiles, query, analytics, plan = null) {
        ...
        const mentionMap = new Map(); // Target -> Set of Source Usernames
        const mentionEvidence = new Map(); // Target -> [Evidence Strings]
        // Pass 1: Build Mention Map
        profiles.forEach(p => {
            const sourceHandle = (p.username || p.ownerUsername || 'unknown').toLowerCase();
            const textToScan = `${p.biography || ''} ${p.latestPosts?.map((post) => post.caption || '').join(' ') || ''}`.toLowerCase();
            ...
            if (mentions) {
                mentions.forEach((m) => {
                    ...
                    if (target !== sourceHandle) {
                        if (!mentionMap.has(target))
                            mentionMap.set(target, new Set());
                        mentionMap.get(target)?.add(sourceHandle);
                        if (!mentionEvidence.has(target))
                            mentionEvidence.set(target, []);
        ...
        if (analytics && analytics.root && analytics.root.children && analytics.root.children.length > 0) {
            ...
            clusters = analytics.root.children.map((aiCluster, idx) => {
            ...
            const bioKeywords = new Map(); // Backup: Bio keywords
            ...
            profiles.forEach(p => {
                ...
                posts.forEach((post) => {
                    ...
                    tags.forEach((t) => uniqueTags.add(t.replace(/^#/, '').toLowerCase()));
                });
                ...
                if (p.biography) {
                    const bioWords = p.biography.toLowerCase().split(/\s+/).filter((w) => w.length > 4 && !w.startsWith('@') && !w.startsWith('http'));
                    const uniqueBio = new Set(bioWords);
                    uniqueBio.forEach((w) => {
                        if (!bioKeywords.has(w))
                            bioKeywords.set(w, []);
            ...
            if (sortedClusters.length > 0) {
                clusters = sortedClusters.map(([tag, members]) => ({
                    ...
                    data: {
                        ...
                        keywords: [tag],
                        provenance: `identified from ${members.length} profiles including: ${members.slice(0, 3).map((m) => '@' + m.username).join(', ')}`
                    },
        ...
        qualityProfiles.forEach((p, index) => {
            ...
            let avgComments = 0;
            if (p.latestPosts && p.latestPosts.length > 0) {
                const totalInteractions = p.latestPosts.reduce((acc, post) => acc + (post.likesCount || 0) + (post.commentsCount || 0), 0);
                const totalLikes = p.latestPosts.reduce((acc, post) => acc + (post.likesCount || 0), 0);
            ...
            const profileHashtags = new Set();
            if (p.latestPosts && Array.isArray(p.latestPosts)) {
                p.latestPosts.forEach((post) => {
                    if (post.hashtags && Array.isArray(post.hashtags)) {
                        post.hashtags.forEach((t) => profileHashtags.add(t.replace(/^#/, '').toLowerCase()));
                    }
                });
            }
            ...
            for (const cluster of clusters) {
                if (cluster.data && cluster.data.keywords && Array.isArray(cluster.data.keywords)) {
                    ...
                    const matchKw = cluster.data.keywords.find((kw) => {
        ...
        root.children = clusters.filter(c => c.children.length > 0).map(cluster => {
            ...
            const totalFollowers = cluster.children.reduce((acc, child) => acc + (child.data?.followerCount || 0), 0);
            const totalFollowing = cluster.children.reduce((acc, child) => acc + (child.data?.followingCount || 0), 0);
            const totalPosts = cluster.children.reduce((acc, child) => acc + (child.data?.postCount || 0), 0);
            ...
            cluster.data = {
                ...
                profiles: cluster.children.map((child) => ({
                    handle: child.label,
                    id: child.id,
        ...
        const traverse = (node, parentId) => {
            ...
            if (node.data && node.data.profiles && Array.isArray(node.data.profiles)) {
                node.data.profiles.forEach((profile) => traverse(profile, node.id));
            }
        ...
        if (extractedAnalytics.creators) {
            extractedAnalytics.creators.sort((a, b) => {
 ...
 */
     ...
     */
    optimizeGraphTopology(nodes, links, intent, root, rawQuery = '') {
        ...
        const scoredNodes = nodes.map(node => {
            ...
            if (context.hasBrand && (node.group === 'brand' || node.type === 'brand'))
                score += 10;
            if (context.hasTopic && (node.group === 'topic' || node.type === 'topic' || node.group === 'hashtag'))
                score += 10;
            if (context.hasGeo && (node.group === 'location' || node.type === 'location' || node.data?.location))
                score += 10;
            if (context.isOverindex && (node.id.includes('overindexed') || node.id.includes('affinity')))
                score += 20;
            // B. Direct Query Match
            if (node.label && q.includes(node.label.toLowerCase()))
                score += 15;
            // C. Evidence Bonus
            // [FIX] Boost evidence score to guarantee inclusion of proven nodes
            if (node.data && node.data.evidence)
                score += 5; // Was 3
            // D. Centrality/Importance (Log Scale)
            if (node.val)
                score += Math.log10(node.val) * 0.5;
            // E. Special Groups
            if (node.group === 'main' || node.group === 'cluster')
                score += 5; // Always keep structural nodes
            if (node.group === 'root')
                score += 100;
        ...
        const highValueClusters = nodes.filter(n => (n.group === 'cluster' || n.group === 'main' || n.group === 'topic' || n.group === 'subtopic' || n.group === 'overindexed')
     ...
     */
    pruneEmptyClusters(nodes, links) {
        let changed = true;
        let pass = 0;
     ...
     */
    generateComparisonGraph(plan, results, query, analytics = null) {
        const nodes = [];
        const links = [];
        const addedNodeIds = new Set();
        ...
        richProfiles.forEach(p => {
            ...
            if (p.id)
                richMap.set(this.normalizeId(p.id), p);
        });
        ...
        results.forEach((resultSet, index) => {
            ...
            const isExplicitEnrichment = currentStep && ((currentStep.actorId && (currentStep.actorId.includes('instagram-api-scraper') || currentStep.actorId.includes('profile-scraper'))) ||
            ...
            resultSet.slice(0, 50).forEach((record, i) => {
                ...
                let group = 'profile';
                if (richProfile.isVerified || (richProfile.followersCount && richProfile.followersCount > 10000))
                    group = 'creator';
                if (!nodeMap.has(nid)) {
                    ...
                    if (analytics) {
                        ...
                        if (newNode.group === 'creator' || newNode.group === 'influencer' || newNode.group === 'profile') {
                            if (analytics.creators)
                                analytics.creators.push(richNode);
                        }
                        else if (newNode.group === 'brand') {
                            if (analytics.brands)
                                analytics.brands.push(richNode);
                        }
                    }
            ...
            if (p.latestPosts && Array.isArray(p.latestPosts)) {
                p.latestPosts.forEach((post) => {
                    ...
                    if (tags.length === 0 && post.caption) {
                        ...
                        captionTags.forEach((t) => keywords.add(t));
                    }
                });
            }
            keywords.forEach((k) => {
        ...
        if (analytics && analytics.visual && analytics.visual.brands) {
            analytics.visual.brands.forEach((brand) => {
        ...
        results.forEach((resultSet, index) => {
            const mainId = `MAIN_${index}`;
            const profiles = [];
            resultSet.slice(0, 50).forEach((record) => {
     ...
     */
    generateCompetitorContentGraph(plan, results, query, analytics = null) {
        const nodes = [];
        const links = [];
        const addedNodeIds = new Set();
        ...
        allPosts.forEach((post, idx) => {
            ...
            if (!postsByType.has(postType))
                postsByType.set(postType, []);
        ...
        const bestHashtagEvidence = new Map();
        const bestMentionEvidence = new Map();
        ...
        return {
            ...
            analytics: {
                ...
                contentAnalysis: {
                    ...
                    postsByType: Object.fromEntries(Array.from(postsByType.entries()).map(([type, posts]) => [
                        type,
                        {
                            count: posts.length,
     ...
     */
    generateHashtagGraph(plan, results, query, analytics = null) {
     ...
     */
    generateEngagementBenchmarkGraph(plan, results, query, analytics = null) {
        const allPosts = results.flat().filter(item => item.type === 'Image' || item.type === 'Video' || item.type === 'Sidecar' || item.caption);
        // Group by owner
     ...
     */
    generateUGCGraph(plan, results, query, analytics = null) {
        ...
        allPosts.forEach((post, idx) => {
            ...
            if (author && author.toLowerCase() !== brandName.toLowerCase()) {
                if (!creators.has(author))
                    creators.set(author, { count: 0, engagement: 0, profilePic: post.owner?.profilePicUrl, posts: [] });
     ...
     */
    generateSentimentGraph(plan, results, query, analytics = null) {
        // Results structure: [0] = posts, [1] = comments (if applicable)
        const posts = results[0] || [];
        const comments = results[1] || [];
        const allTextItems = [
            ...posts.map((p) => ({ text: p.caption, type: 'post', engagement: (p.likesCount || 0) + (p.commentsCount || 0), url: p.url })),
            ...comments.map((c) => ({ text: c.text, type: 'comment', engagement: c.likesCount || 0, url: proxyMediaUrl(c.ownerProfilePicUrl) }))
        ].filter(i => i.text);
        ...
        allTextItems.forEach(item => {
            ...
            words.forEach((w) => {
                ...
                if (w.length > 4 && !positiveWords.has(w) && !negativeWords.has(w)) {
                    if (!keyThemes.has(w))
                        keyThemes.set(w, { count: 0, sentiment: 0 });
     ...
     */
    buildGraphHierarchy(flatNodes, flatLinks = []) {
        ...
        const leafNodes = flatNodes.filter(n => ['brand', 'creator', 'topic', 'profile', 'post', 'concept'].includes(n.group));
        if (!mainNode) {
        ...
        const assignedLeafIds = new Set(); // [FIX] Declaration moved up for scope visibility
        clusterNodes.forEach(cluster => {
            ...
            evidence.forEach((ev) => {
                if (ev.author)
                    mentionedUsernames.add(ev.author.toLowerCase());
                if (ev.username)
                    mentionedUsernames.add(ev.username.toLowerCase());
            });
            ...
            leafNodes.forEach(leaf => {
                ...
                const isMatch = Array.from(mentionedUsernames).some(mentioned => {
                    ...
                    return cleanMention.length > 2 && (leafUsername.includes(cleanMention) ||
        ...
        if (orphanedLeaves.length > 0 && clusterNodes.length > 0) {
            ...
            orphanedLeaves.forEach((leaf, idx) => {
                ...
                for (const cluster of clusterNodes) {
                    ...
                    const clusterKeywords = (cluster.data?.keywords || []).map((k) => k.toLowerCase());
                    let score = 0;
                    if (leafText.includes(clusterText))
                        score += 3;
                    clusterKeywords.forEach((kw) => {
                        if (leafText.includes(kw))
                            score += 1;
                    });
                    if (score > maxScore) {
                        maxScore = score;
     ...
     */
    generateInfluencerGraph(plan, results, query, analytics = null) {
        ...
        const aiMatchers = new Map();
        if (analytics && analytics.creators) {
            analytics.creators.forEach((c) => {
                if (c.handle)
                    aiMatchers.set(c.handle.toLowerCase(), c);
            });
        }
     ...
     */
    generateViralGraph(plan, results, query, analytics = null) {
        const posts = results[0] || [];
        const nodes = [];
        const links = [];
        const centralLabel = plan.search_keywords?.[0] || query;
     ...
     */
    aggregateOverindexingLocal(results, intent) {
        // 1. Identify "Audience" steps vs "Enrichment" steps
        let enrichmentData = [];
        const audienceBatches = [];
        ...
        audienceBatches.forEach((batch, batchIdx) => {
            batch.forEach((rawItem) => {
                ...
                if (username) {
                    ...
                    if (merged.biography)
                        merged.bio = merged.biography;
                    merged.followers = merged.followersCount || merged.followerCount || rawItem.followers || 0;
                    if (merged.profile_pic_url)
                        merged.profilePicUrl = merged.profile_pic_url;
    ...
    generateDynamicClusters(profiles, sampleSize) {
        ...
        profiles.forEach(p => {
            ...
            posts.forEach((post) => {
                if (post.hashtags && Array.isArray(post.hashtags)) {
                    post.hashtags.forEach((t) => uniqueTags.add(t.replace(/^#/, '').toLowerCase()));
                }
            });
            uniqueTags.forEach(tag => {
            ...
            if (p.biography) {
                const words = p.biography.toLowerCase()
                    .replace(/[^\w\s]/g, '')
                    .split(/\s+/)
                    .filter((w) => w.length > 4 && !['instagram', 'follow', 'dm', 'email', 'contact'].includes(w));
                const uniqueWords = new Set(words);
                uniqueWords.forEach(w => {
                    const wordStr = String(w);
                    if (!bioKeywords.has(wordStr))
                        bioKeywords.set(wordStr, []);
        ...
        if (clusters.length === 0) {
            clusters.push({ id: 'c_top', label: 'Top Creators', type: 'cluster', group: 'cluster', val: 15, data: { keywords: ['creator', 'official', 'verified'] }, children: [] }, // 20 -> 15
     ...
     */
    async generateOverindexGraph(profiles, centralLabel, plan = null, analytics = null, sampleSize = 100) {
        const nodes = [];
        const links = [];
        ...
        if (isComparison) {
            ...
            if (handleMatch)
                displayLabel = `@${handleMatch[1]}`;
            else
                displayLabel = centralLabel.replace(/map (the )?over-indexed creators (for|of) (followings of followers of )?/i, '').trim();
        }
        ...
        profiles.forEach(p => {
            ...
            if (username && !profileMap.has(username)) {
                ...
                if (!existing.biography && p.biography)
                    profileMap.set(username, p);
            }
        });
        ...
        if (mainNode && coreProfileData) {
            ...
            mainNode.data = {
                ...
                latestPosts: (coreProfileData.latestPosts || []).map((p) => proxyMediaFields(p))
            };
        }
        ...
        const uniqueInteractions = new Map(); // Target -> Set<SourceID>
        const realSourceMap = new Map(); // Target -> Array<SourceHandles>
        let totalSourceProfiles = 0;
        // Helper to add interaction
        const registerInteraction = (target, source, type) => {
            const cleanTarget = normalizeId(target);
            const cleanSource = normalizeId(source);
            if (!cleanTarget || !cleanSource || cleanTarget === normalizeId(coreId))
                return;
            if (!uniqueInteractions.has(cleanTarget))
                uniqueInteractions.set(cleanTarget, new Set());
            uniqueInteractions.get(cleanTarget).add(cleanSource);
            // Add Evidence (limit to 20 unique sources for UI)
            if (!realSourceMap.has(cleanTarget))
                realSourceMap.set(cleanTarget, []);
        ...
        profiles.forEach(p => {
            ...
            if (sourceHandle === 'Unknown' || !sourceHandle)
                return;
            // 1. Process Follows
            if (p.follows && Array.isArray(p.follows)) {
                p.follows.forEach((followedAccount) => {
        ...
        const realFrequencyMap = new Map();
        ...
        const scanTreeFrequencies = (node) => {
            if (!node)
                return;
            ...
            if (node.children) {
                node.children.forEach((c) => scanTreeFrequencies(c));
            }
        };
        if (analytics && analytics.root) {
        ...
        const processNode = (treeNode, parentId, inferredGroup = null) => {
            if (!treeNode)
                return;
            ...
            if (group === 'cluster' || group === 'subcluster' || group === 'category' || group === 'theme')
                nodeColor = '#10b981'; // Emerald Green
            else if (group === 'topic')
                nodeColor = '#10b981'; // [FIX] Clusters are Green. Topics = Clusters.
            else if (group === 'creator' || group === 'profile')
                nodeColor = '#f472b6'; // Pink
            else if (group === 'brand')
                nodeColor = '#3b82f6'; // Blue
            else if (group === 'post')
                nodeColor = '#f1f5f9'; // Slate
            if (!addedNodeIds.has(nodeId)) {
                ...
                if ((group === 'cluster' || group === 'topic' || group === 'subtopic') && treeNode.children) {
                    childEvidence = treeNode.children.slice(0, 15).map((child) => {
                ...
                if (!finalUrl && treeNode.data?.externalUrl)
                    finalUrl = treeNode.data.externalUrl;
                ...
                const richData = {
                    ...
                    latestPosts: realPosts.length > 0 ? realPosts.map(p => proxyMediaFields(p)) : (treeNode.data?.latestPosts ? treeNode.data.latestPosts.map((p) => proxyMediaFields(p)) : []),
                ...
                if (treeNode.children && Array.isArray(treeNode.children)) {
                    treeNode.children.forEach((child) => processNode(child, nodeId, group === 'cluster' ? 'creator' : inferredGroup));
                }
                ...
                if (treeNode.data && treeNode.data.profiles && Array.isArray(treeNode.data.profiles)) {
                    ...
                    treeNode.data.profiles.forEach((profile) => processNode(profile, nodeId, profileGroup));
                }
            }
                ...
                if (isLeaf && currentNode) {
                    ...
                    // 2. SEMANTIC HOMING: Try to find a better cluster match if currently pointing to MAIN
                        ...
                        nodes.filter(n => n.group === 'cluster' || n.group === 'topic').forEach(cluster => {
                            if (cluster.id === highAffinityClusterId)
                                return;
                            const keywords = cluster.data?.keywords || (cluster.label ? cluster.label.split(' ') : []);
                            const matches = keywords.filter((k) => nodeBio.includes(k.toLowerCase())).length;
        ...
        if (analytics && analytics.root) {
            ...
            if (analytics.root.children && analytics.root.children.length > 0) {
                analytics.root.children.forEach((cluster) => {
            ...
            if (hasHighAffinity) {
                ...
                const overindexedMembers = nodes.filter(n => links.some(l => l.source === highAffinityClusterId && l.target === n.id));
                const topicCounts = new Map();
                const topicEvidence = new Map();
                overindexedMembers.forEach(m => {
                    ...
                    keywords.forEach((k) => {
                        ...
                        if (topic.length > 2) {
                            ...
                            if (!topicEvidence.has(topic))
                                topicEvidence.set(topic, { source: m.id, bio });
                        }
                    });
                    // Add post hashtags
                ...
                topTopics.forEach(([topic, count]) => {
                    ...
                    overindexedMembers.forEach(m => {
                        const bio = (m.data?.bio || '').toLowerCase();
                        const posts = m.data?.latestPosts || [];
                        const hasTopic = bio.includes(topic) || posts.some((p) => (p.caption || '').includes(topic));
                        if (hasTopic) {
            ...
            if (analytics && analytics.visual) {
                    ...
                    analytics.visual.brands.forEach((brand) => {
                        ...
                        let richData = {
                            ...
                            provenance: {
                                ...
                                evidence: brand.imageUrls?.map((url) => ({
                            ...
                            evidenceItems: brand.imageUrls?.map((url) => ({
                                type: 'visual',
                                label: 'Visual Match',
                        ...
                        if (scrapedProfile) {
                            richData = {
                                ...
                                latestPosts: (scrapedProfile.latestPosts || []).map((p) => proxyMediaFields(p)),
                                isVerified: scrapedProfile.isVerified || false
                            };
                        }
                        nodes.push({
                ...
                if (analytics.visual.products && Array.isArray(analytics.visual.products)) {
                    analytics.visual.products.forEach((product, idx) => {
                ...
                if (analytics.visual.colorPalette && analytics.visual.colorPalette.length > 0) {
                    extractedAnalytics.visualTheme = {
                    ...
                    console.log("[GraphGen] Mapped Visual Theme:", extractedAnalytics.visualTheme);
                }
            }
            ...
            sortedOverindexed.forEach(([username, frequency]) => {
                ...
                if (addedNodeIds.has(nodeId))
                    return; // Already added via AI tree
                ...
                // [UNIFIED] Use central hydration helper
                const hydrated = this.hydrateNodeData(profile, profile.isBusinessAccount ? 'brand' : 'creator', `High Affinity: Followed by ${frequency} source accounts`, centralLabel);
                ...
                const profileKeywords = new Set([
                    ...
                    ...(profile.latestPosts?.flatMap((p) => p.hashtags || []) || []).map((h) => h.toLowerCase().replace('#', ''))
                ]);
                nodes.filter(n => n.group === 'cluster' || n.group === 'topic').forEach(cluster => {
                    const clusterKeywords = cluster.data?.keywords || (cluster.label ? cluster.label.toLowerCase().split(' ') : []);
                    const matches = clusterKeywords.filter((k) => profileKeywords.has(k.toLowerCase())).length;
                    if (matches > maxMatches) {
                        maxMatches = matches;
        ...
        leafNodes.forEach(node => {
            ...
            const keywords = bio.split(/\W+/).filter((w) => w.length > 4); // Filter short words
        ...
        leafNodes.forEach((nodeA, i) => {
            ...
            for (let j = i + 1; j < leafNodes.length; j++) {
                ...
                for (const k of keywordsA) {
                    if (keywordsB.includes(k))
                        matchCount++;
                    if (matchCount >= 5)
                        break; // Optimization: Stop if max score reached
                }
                let overlapScore = 0;
                if (matchCount >= 3)
                    overlapScore += 1;
        ...
        if (extractedAnalytics && mainNodeForVisuals && mainNodeForVisuals.data && mainNodeForVisuals.data.visualIdentity) {
            extractedAnalytics.visual = mainNodeForVisuals.data.visualIdentity;
        }
        ...
        for (const node of nodes) {
            ...
            const isPlaceholderBio = bioText.includes('bio unavailable') ||
                ...
                bioText.length < 5;
            const hasEnrichedData = node.data && ((node.data.followerCount && node.data.followerCount > 0) ||
        ...
        if (nodesToEnrich.length > 0) {
            ...
            for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                ...
                try {
                    ...
                    const scrapedProfiles = await this.runApifyActor('apify/instagram-profile-scraper', {
                        usernames,
                        ...
                        for (const item of batch) {
                            ...
                            if (scrapedProfile) {
                                ...
                                if (scrapedProfile.latestPosts && scrapedProfile.latestPosts.length > 0) {
                                    item.node.data.latestPosts = scrapedProfile.latestPosts.map((post) => proxyMediaFields(post));
                                }
     ...
     */
    generateClusterGraph(profiles, centralLabel, sampleSize = 100) {
        const nodes = [];
        const links = [];
        // Dynamic Limits
        ...
        const profilesInClusters = new Set();
        clusters.forEach((cluster, idx) => {
            ...
            cluster.members.slice(0, membersPerCluster).forEach((profile) => {
                const username = profile.username;
                if (!username || profilesInClusters.has(username))
                    return;
    ...
    // 1. Geo Graph
    generateGeoGraph(records, centralLabel, geoData, sampleSize = 100) {
        const nodes = [];
        const links = [];
        // Dynamic Limits
    ...
    determineNodeGroup(username, analytics) {
        if (!username)
            return 'profile';
        ...
        if (analytics) {
            // Check Brands
            if (analytics.brands && analytics.brands.some((b) => (b.username && b.username.toLowerCase() === clean) ||
                (b.name && b.name.toLowerCase() === clean))) {
                return 'brand';
            }
            // Check Creators
            if (analytics.creators && analytics.creators.some((c) => (c.username && c.username.toLowerCase() === clean))) {
                return 'creator';
            }
            // Check Overindexed
            if (analytics.overindexedAccounts && analytics.overindexedAccounts.some((o) => (o.username && o.username.toLowerCase() === clean))) {
                const match = analytics.overindexedAccounts.find((o) => o.username.toLowerCase() === clean);
                if (match && match.category === 'brand')
                    return 'brand';
                return 'overindexed';
            }
        }
    ...
    handleCompetitorContentGraph(plan, results, query) {
    ...
    handleHashtagGraph(plan, results, query) {
        ...
        return {
            analytics: {
                ...graphData.analytics,
                topContent: graphData.nodes
                    .filter((n) => n.group === 'post' || n.data.url)
                    .map((n) => ({
                    title: n.label,
                    platform: 'Instagram',
                    url: n.data.url,
                ...
                topics: graphData.nodes
                    .filter((n) => n.group === 'topic' && n.id !== 'MAIN_HASHTAG')
                    .map((n) => ({
                    name: n.label,
    ...
    handleEngagementBenchmarkGraph(plan, results, query) {
        ...
        return {
            analytics: {
                ...graphData.analytics,
                comparison: {
                    profiles: graphData.analytics.benchmarkAnalysis.profiles.map((p) => ({
                        name: p.username,
    ...
    handleUGCGraph(plan, results, query) {
        ...
        return {
            analytics: {
                ...graphData.analytics,
                overindexing: {
                    topCreators: graphData.analytics.ugcAnalysis.topCreators.map((c) => ({
                        name: c.name,
                        handle: c.handle,
                        score: 10,
    ...
    handleSentimentGraph(plan, results, query) {
    ...
    handleInfluencerGraph(plan, results, query) {
    ...
    handleViralGraph(plan, results, query) {
    ...
    handleComparisonGraphHandler(plan, results, query) {
