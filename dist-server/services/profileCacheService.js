 ...
 */
export const getCachedFollowerCount = async (username, platform) => {
    try {
        const response = await fetch(`${API_BASE}/profile-cache/${platform}/${encodeURIComponent(username)}`);
 ...
 */
export const cacheFollowerCount = async (username, platform, followers) => {
    try {
 ...
 */
export const getFollowerCountWithCache = async (username, platform, fetchFn) => {
    // Check cache first
