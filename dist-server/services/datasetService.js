 ...
 */
const generateAutoTags = (dataset) => {
    const tags = [];
    if (dataset.platform)
        tags.push(dataset.platform);
    if (dataset.dataType)
        tags.push(dataset.dataType);
    if (dataset.recordCount) {
        if (dataset.recordCount >= 10000)
            tags.push('large');
        else if (dataset.recordCount >= 1000)
            tags.push('medium');
        else
            tags.push('small');
    }
...
export const createDataset = async (data) => {
    ...
    if (!res.ok)
        throw new Error(`Failed to create dataset: ${res.statusText}`);
    return dataset;
};
/**
 * Add new data source to an existing dataset (Composite Dataset)
 */
export const addToDataset = async (datasetId, newData, newSource) => {
    ...
    if (!res.ok)
        throw new Error(`Failed to update dataset: ${res.statusText}`);
 ...
 */
export const searchDatasets = async (criteria) => {
    ...
    if (criteria.platform)
        options.platform = criteria.platform;
    ...
    return candidates.filter(dataset => {
        if (criteria.targetProfile) {
            ...
            if (!dataset.targetProfile.toLowerCase().includes(searchProfile))
                return false;
        }
        if (criteria.dataType && dataset.dataType !== criteria.dataType)
            return false;
        if (criteria.project && dataset.project !== criteria.project)
            return false;
...
export const recordQueryUsage = async (datasetId, query) => {
    const dataset = await getDataset(datasetId);
    if (!dataset)
        return;
...
export const getDatasetContext = (dataset, limit = 20) => {
...
export const findProfileAnalytics = async (username) => {
    ...
    for (const dataset of candidates) {
        const record = dataset.data.find((item) => {
            const itemUser = norm(item.username || item.ownerUsername || item.id);
            return itemUser === target;
        });
        if (record)
            return { ...record, fullName: record.fullName || record.full_name };
    }
    return null;
};
export const getBatchProfileAnalytics = async (usernames) => {
 ...
 */
export const importDatasets = async (jsonString) => {
    try {
        const datasets = JSON.parse(jsonString);
        let importCount = 0;
