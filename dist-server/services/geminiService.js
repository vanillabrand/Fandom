...
export const getFollowerCount = async (platform, profile) => {
...
export const getProfileImage = async (platform, profile) => {
    const APIFY_API_TOKEN = (typeof window !== 'undefined' && window.__ENV__?.APIFY_API_TOKEN)
        ? window.__ENV__.APIFY_API_TOKEN
...
export const fetchFandomAnalysis = async (profile, platform, sampleSize) => {
    ...
    try {
        ...
        if (response && response.text) {
            ...
            try {
                ...
                rawData.analytics.clusters.forEach((cluster, i) => {
                    const cid = `c_${i}`;
                    nodes.push({
                        id: cid,
                        label: cluster.name,
                    ...
                    const creatorsForCluster = rawData.analytics.creators.filter((_, idx) => idx % rawData.analytics.clusters.length === i);
                ...
                rawData.analytics.topics.forEach((topic, i) => {
                    const tid = `t_${i}`;
                    nodes.push({
                        id: tid,
                        label: topic.name,
                ...
                rawData.analytics.brands.slice(0, 8).forEach((brand, i) => {
                    const bid = `b_${i}`;
                ...
                rawData.analytics.nonRelatedInterests.forEach((interest, i) => {
                    const nid = `nri_${i}`;
                    nodes.push({
                        id: nid,
                        label: interest.name,
                ...
                const hasCompleteProvenance = (item) => {
                    if (!item)
                        return false;
                ...
                const uniqueCreators = Array.from(new Map(rawData.analytics.creators
                    .filter((c) => isEnglishText(c.name) && isEnglishText(c.handle) && hasCompleteProvenance(c))
                    .map((c) => [c.handle.toLowerCase(), c])).values());
                ...
                const uniqueBrands = Array.from(new Map(rawData.analytics.brands
                    .filter((b) => isEnglishText(b.name) && hasCompleteProvenance(b))
                    .map((b) => [b.name.toLowerCase(), b])).values());
                ...
                const uniqueClusters = Array.from(new Map(rawData.analytics.clusters
                    .filter((c) => isEnglishText(c.name) && hasCompleteProvenance(c))
                    .map((c) => [c.name.toLowerCase(), c])).values());
                ...
                const uniqueNonRelated = Array.from(new Map(rawData.analytics.nonRelatedInterests
                    .filter((n) => isEnglishText(n.name) && hasCompleteProvenance(n))
                    .map((n) => [n.name.toLowerCase(), n])).values());
...
export const analyzeBatch = async (posts, query, analysisType = 'standard') => {
    const model = "gemini-3-flash-preview";
...
export const aggregateLocations = async (locations) => {
    const model = "gemini-3-flash-preview";
    // Filter empty
...
const getVisualAnalysisPrompt = (analysisType) => {
...
export const analyzeVisualContent = async (imageUrls, analysisType = 'full') => {
    ...
    try {
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            const imageParts = await Promise.all(batch.map(url => fetchImageAsBase64(url)));
        ...
        allResults.forEach(result => {
            if (result.aestheticTags) {
                result.aestheticTags.forEach((tag) => {
...
export const analyzeFandomDeepDive = async (query, scrapedData, intent = 'general', platform = 'instagram', datasetUrl = '', sampleSize = 100, useVisualTheme = false, richContext = [], // [NEW] Master List of Full Profiles for Hydration
mode = 'full', seedContext = "" // [NEW] Accept Seed Context
) => {
    console.log(`[GeminiService] Starting Analysis: ${query} (Intent: ${intent}, Mode: ${mode}, Items: ${scrapedData.length})`);
    if (seedContext)
        console.log(`[GeminiService] Mixing in Seed Context (${seedContext.length} chars)`);
    ...
    `;
        ...
        let visualAnalysisPromise = null;
        if (useVisualTheme) {
            const candidateImages = scrapedData
                ...
                if (item.latestPosts && Array.isArray(item.latestPosts)) {
                    item.latestPosts.slice(0, 1).forEach((p) => {
                        if (p.displayUrl)
                            images.push(p.displayUrl);
                        if (p.url && !p.url.includes('tiktok'))
                            images.push(p.url); // Avoid video links for vision
                    });
                }
                if (item.profilePicUrl)
                    images.push(item.profilePicUrl);
                return images;
            })
...
export const verifyAnalysis = async (query, analytics) => {
    try {
        ...
        if (analytics) {
            ...
            if (analytics.clusters && analytics.clusters.length > 0)
                presentFields++;
        ...
        if (clusterCount >= 5)
            qualityMetrics.diversity += 40;
        else if (clusterCount >= 3)
            qualityMetrics.diversity += 25;
        else
            issues.push(`Low cluster diversity: only ${clusterCount} clusters`);
        if (creatorCount >= 10)
            qualityMetrics.diversity += 30;
        else if (creatorCount >= 5)
            qualityMetrics.diversity += 15;
        if (brandCount >= 10)
            qualityMetrics.diversity += 30;
        ...
        const summary = `
        Clusters: ${analytics.clusters?.map((c) => c.label).join(', ') || 'None'}
        Top Creators: ${analytics.creators?.slice(0, 5).map((c) => c.label).join(', ') || 'None'}
        Top Brands: ${analytics.brands?.slice(0, 5).map((c) => c.label).join(', ') || 'None'}
        Topics: ${analytics.topics?.slice(0, 5).map((c) => c.label).join(', ') || 'None'}
        ...
        **TASK:**
        ...
        if (response.text) {
            ...
            if (qualityMetrics.completeness < 50)
                adjustedConfidence -= 15;
            if (qualityMetrics.diversity < 50)
...
const getPromptForMode = (mode, intent, query, platform, targetCount, useVisualTheme, focusCategory) => {
...
const getIntentSpecificInstructions = (intent, query, platform, targetCount = 20, useVisualTheme = false) => {
    switch (intent) {
...
export const generateMarketingQuestionsPrompt = () => {
    return `
