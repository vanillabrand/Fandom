 ...
 */
const aggregateFollowing = (followingSamples) => {
 ...
 */
const calculateOverindexScore = (frequencyInSample, sampleSize, baselineFrequency = 0.01 // Default 1% baseline
) => {
 ...
 */
export const calculateOverindexing = async (targetProfile, platform, followersDatasetId, followingDatasetIds = [], rawFollowers, // Optimized: Pass data directly
rawFollowing // Optimized: Pass data directly
) => {
    ...
    const followingSamples = rawFollowing || [];
    ...
    if (followingSamples.length === 0 && followersData && followersData.length > 0) {
        ...
        if (sampleEx.caption || sampleEx.taken_at || sampleEx.media_type || sampleEx.text) {
            ...
            posts.forEach((post) => {
                ...
                if (!owner && post.owner && post.owner.username)
                    owner = post.owner.username;
                if (!owner && post.user && post.user.username)
                    owner = post.user.username;
                if (!owner || owner === 'unknown')
                    return;
                owner = owner.toLowerCase();
                if (!interactionMap.has(owner))
                    interactionMap.set(owner, new Set());
                ...
                mentions.forEach((m) => interactions.add(m.replace('@', '').toLowerCase()));
                // 2. Extract from Tagged Users (Nested check)
                if (post.taggedUsers && Array.isArray(post.taggedUsers)) {
                    post.taggedUsers.forEach((u) => {
                        if (typeof u === 'string')
                            interactions.add(u.toLowerCase());
                        else if (u.username)
                            interactions.add(u.username.toLowerCase());
                        else if (u.user && u.user.username)
                            interactions.add(u.user.username.toLowerCase());
                    });
                }
                // 3. Extract from Mentions Array (if available)
                if (post.mentions && Array.isArray(post.mentions)) {
                    post.mentions.forEach((u) => interactions.add(u.toLowerCase()));
                }
            });
    ...
    // 3. Process the data
    return processFollowingForOverindexing(targetProfile, platform, followingSamples);
};
/**
 * Process raw following data from multiple followers to find over-indexed accounts
 */
export const processFollowingForOverindexing = (targetProfile, platform, followingSamples, minFrequency = 2, // Minimum threshold: Must appear at least 2 times
topN = 50 // Return top N results per category
) => {
    ...
    const totalFollowingAnalyzed = followingSamples.reduce((sum, sample) => sum + sample.length, 0);
    return {
        targetProfile,
        platform,
 ...
 */
export const findClusters = (overindexedAccounts, followingSamples, minClusterSize = 3) => {
        ...
        if (accountCoOccurrence) {
            accountCoOccurrence.forEach((count, otherUsername) => {
                if (count >= threshold && !used.has(otherUsername)) {
                    const otherAccount = overindexedAccounts.find(a => a.username.toLowerCase() === otherUsername);
                    if (otherAccount) {
 ...
 */
export const saveOverindexingResults = async (results, name) => {
    ...
    return createDataset({
        ...
        metadata: {
            ...
            estimatedCompleteness: Math.min(100, (results.followersSampled / 1000) * 100 // 1000 followers = 100% completeness
            )
        },
        sources: []
    });
 ...
 */
export const getOverindexedFromDataset = async (targetProfile, platform) => {
