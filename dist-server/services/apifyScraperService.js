...
const localDatasetCache = new Map();
// Helper function to generate fingerprint from scrape request
const generateFingerprintForScrape = async (request) => {
...
const checkExistingFingerprint = async (fingerprint) => {
    try {
...
const getApifyToken = () => {
    ...
    if (typeof window !== 'undefined' && window.__ENV__?.APIFY_API_TOKEN) {
        return window.__ENV__.APIFY_API_TOKEN;
    }
...
const getCloudRunUrl = () => {
    ...
    if (typeof window !== 'undefined' && window.__ENV__?.VITE_CLOUDRUN_SCRAPER_URL) {
        return window.__ENV__.VITE_CLOUDRUN_SCRAPER_URL;
    }
 ...
 */
const fallbackToCloudRun = async (request) => {
 ...
 */
export const getRecommendedActors = (platform, dataTypes) => {
    return ACTOR_REGISTRY.filter(actor => actor.platform === platform &&
        dataTypes.some(dt => actor.dataTypes.includes(dt))).sort((a, b) => {
 ...
 */
export const getBestActor = (request) => {
 ...
 */
export const estimateCost = (actorId, limit) => {
 ...
 */
const buildActorInput = (actor, request) => {
 ...
 */
export const startScrapeJob = async (request) => {
    ...
    const response = await fetch(`/apify-api/v2/acts/${apiActorId}/runs?memory=${memoryMbytes}`, {
        method: 'POST',
        headers,
 ...
 */
export const pollRunStatus = async (runId) => {
    const token = getApifyToken();
    const response = await fetch(`/apify-api/v2/actor-runs/${runId}`, {
        headers: {
 ...
 */
export const waitForCompletion = async (runId, onProgress, maxWaitMinutes = 30, pollIntervalMs = 3000) => {
 ...
 */
export const fetchResults = async (datasetId) => {
    ...
    const token = getApifyToken();
    const response = await fetch(`/apify-api/v2/datasets/${datasetId}/items`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    });
 ...
 */
export const normalizeData = (rawData, platform, dataType) => {
 ...
 */
export const scrapeFollowers = async (platform, username, limit = 1000, onProgress) => {
    const request = {
        platform,
 ...
 */
export const scrapeFollowing = async (platform, username, limit = 1000, onProgress) => {
    const request = {
        platform,
        targetProfile: username,
        dataType: 'following',
 ...
 */
export const enrichProfiles = async (platform, usernames, onProgress) => {
 ...
 */
export const scrapePosts = async (platform, username, limit = 50, onProgress) => {
    const request = {
        platform,
        targetProfile: username,
        dataType: 'posts',
...
export const validateToken = async () => {
    try {
        ...
        if (token)
            headers['Authorization'] = `Bearer ${token}`;
        const response = await fetch('/apify-api/v2/users/me', { headers });
        return response.ok;
    }
    catch {
        return false;
