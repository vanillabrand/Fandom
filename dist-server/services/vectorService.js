...
const getAiClient = () => {
    if (!aiClient) {
        const apiKey = (typeof window !== 'undefined' && window.__ENV__?.GEMINI_API_KEY)
 ...
 */
const extractTextFromRecord = (record, type) => {
    const parts = [];
    if (type === 'posts') {
        if (record.caption)
            parts.push(record.caption);
        if (record.hashtags && Array.isArray(record.hashtags))
            parts.push(record.hashtags.join(' '));
        if (record.location)
            parts.push(typeof record.location === 'string' ? record.location : record.location.name);
    }
    else if (type === 'followers' || type === 'following' || type === 'profiles' || type === 'overindexed') {
        if (record.fullName)
            parts.push(record.fullName);
        if (record.biography || record.bio)
            parts.push(record.biography || record.bio);
        if (record.category)
 ...
 */
export const indexDataset = async (dataset) => {
    // Extract texts from all records
    const texts = dataset.data.map(record => extractTextFromRecord(record, dataset.dataType === 'composite' ? record.recordType : dataset.dataType));
 ...
 */
export const searchDataset = async (dataset, query, topK = 10) => {
 ...
 */
export const searchAllDatasets = async (datasets, query, topK = 20) => {
    ...
    // 2. Parallel Search
    const searchPromises = enabledDatasets.map(d => searchDataset(d, query, topK)
