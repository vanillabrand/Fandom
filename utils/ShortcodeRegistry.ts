export class UniversalShortcodeRegistry {
     ...
     */
    register(value: string | undefined | null): string {
        if (!value) return '';

        // Don't register short values (e.g. "US") - not worth the overhead
        if (value.length < 15) return value; // Arbitrary threshold
     ...
     */
    getStats(): { items: number, saving: number } {
        let originalSize = 0;
