...
export const safeParseJson = (text: string): any => {
    ...
    for (let i = 0; i < candidate.length; i++) {
        ...
        if (escape) { sanitized += char; escape = false; continue; }
        if (char === '\\') { sanitized += char; escape = true; continue; }
        ...
        if (!inString) {
            ...
            if (char === '/' && candidate[i + 1] === '*') {
                i += 2;
                while (i < candidate.length && !(candidate[i] === '*' && candidate[i + 1] === '/')) i++;
                i++;
                continue;
            }
            if (char === '#') {
...
const attemptRobustParse = (text: string): any => {
    ...
    try {
        ...
        try {
            ...
            for (let i = 0; i < text.length; i++) {
                const char = text[i];
                if (escape) { escape = false; repaired += char; continue; }
                if (char === '\\') { escape = true; repaired += char; continue; }
