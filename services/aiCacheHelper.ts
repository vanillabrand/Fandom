...
export interface CachedGeminiOptions {
    ...
    prompt: string;
    config?: any;
    cacheTTL?: number; // Hours, default 24
    bypassCache?: boolean; // Force fresh API call
}

export interface CachedGeminiResponse {
    text: string;
    fromCache: boolean;
    cost?: number;
}
 ...
 */
export async function invalidateAICache(modelPattern?: string): Promise<number> {
    try {
