...
export interface QueryQualityScore {
    ...
    relevance: number;         // How well it matches query
    freshness: number;         // Data recency score
    provenance: number;        // % nodes with source attribution
}

export interface QueryConfidence {
    score: number;             // 0-100 confidence level
    factors: string[];         // Factors affecting confidence
    lowConfidenceAreas: string[]; // Areas flagged for review
}

export interface QueryFeedback {
    helpful: boolean | null;
    rating: number | null;     // 1-5 stars
    categories: string[];      // e.g., ["low_relevance", "missing_data"]
    comment: string;
}
...
export interface QuerySuggestion {
    ...
    suggestion: string;
    reasoning: string;
    expectedImprovement: number; // 0-100 expected quality increase
}

/**
 * Calculate overall quality score for a query result
 */
export const scoreQueryResult = (
    query: string,
    result: any,
    metadata: any
): QueryQualityScore => {
    const scores: QueryQualityScore = {
        overall: 0,
        completeness: 0,
        relevance: 0,
        freshness: 0,
    ...
    if (result.analytics) {
        ...
        if (result.analytics.clusters) presentFields++;
        if (result.analytics.creators) presentFields++;
        if (result.analytics.brands) presentFields++;
        if (result.analytics.topics) presentFields++;
        if (result.root) presentFields++;
    }
    ...
    if (metadata?.createdAt) {
        ...
        if (ageInDays <= 1) scores.freshness = 100;
        else if (ageInDays <= 7) scores.freshness = 90;
        else if (ageInDays <= 30) scores.freshness = 70;
        else if (ageInDays <= 90) scores.freshness = 50;
        else scores.freshness = 30;
    } else {
 ...
 */
export const trackQueryFeedback = async (
    jobId: string,
    userId: string,
    query: string,
...
export const getQuerySuggestions = (
    query: string,
    result: any,
    qualityScore: QueryQualityScore
): QuerySuggestion[] => {
 ...
 */
export const identifyLowConfidenceAreas = (result: any): string[] => {
    const issues: string[] = [];
