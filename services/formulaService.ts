...
interface EnrichedDataPoint {
    id: string;
    source: 'instagram' | 'tiktok' | 'unknown';
    author: string;
    text?: string;
    likes: number;
    comments: number;
    ...
    url: string;
    timestamp?: string;
    username?: string; // [FIX] Preserved for orchestration
    profileId?: string; // [FIX] Preserved ID
}
...
export const FormulaService = {
     ...
     */
    normalizeData: (actorId: string, item: any): EnrichedDataPoint => {
        // Default structure
        const normalized: EnrichedDataPoint = {
        ...
        if (actorId.includes('followers') || actorId.includes('following') || actorId.includes('thenetaji')) {
            ...
            if (item.timestamp) normalized.timestamp = item.timestamp;
        }
     ...
     */
    calculate: (formula: string, data: EnrichedDataPoint): number => {
     ...
     */
    calculateAggregate: (formula: string, dataset: EnrichedDataPoint[]): number => {
        if (!dataset || dataset.length === 0) return 0;

        const sum = (field: keyof EnrichedDataPoint) => dataset.reduce((acc, curr) => acc + (Number(curr[field]) || 0), 0);
        const avg = (field: keyof EnrichedDataPoint) => sum(field) / dataset.length;
     ...
     * Returns % difference
     */
    compare: (metric: string, target: EnrichedDataPoint[], benchmark: EnrichedDataPoint[]): number => {
