...
import { searchDatasets, getDatasetSummaries, getDataset } from './datasetService.js';
 ...
 */
export const preprocessQuery = (input: string): string => {
    if (!input) return "";
...
export const analyzeQuery = (query: string): QueryAnalysis => {
    ...
    const requiredDataTypes: DatasetType[] = [];
    ...
    for (const [dataType, patterns] of Object.entries(INTENT_PATTERNS)) {
        ...
        if (score > 0) {
            requiredDataTypes.push(dataType as DatasetType);
    ...
    let platform: DatasetPlatform | undefined;
 ...
 */
export const findMatchingDatasets = async (
    ...
    for (const dataset of allDatasets) {
        ...
        const daysSinceCreation = Math.floor(
            (Date.now() - new Date(dataset.createdAt).getTime()) / (1000 * 60 * 60 * 24)
        );
...
export const calculateSuccessProbability = (
    ...
    if (query.targetProfile) {
        const exactMatch = matches.some(
            m => m.dataset.targetProfile.toLowerCase() === query.targetProfile?.toLowerCase()
        );
        if (exactMatch) {
            probability += 15;
 ...
 */
export const suggestApproaches = (
    query: QueryAnalysis,
    matches: DatasetMatch[],
    accuracy: AccuracyIndicator
): ApproachSuggestion[] => {
    ...
    if (accuracy.score < 40 && query.targetProfile) {
        ...
        const missingTypes = query.requiredDataTypes.filter(
            dt => !matches.some(m => m.dataset.dataType === dt)
        );
...
export const validateQuery = async (
    ...
    const datasetAge = matchingDatasets.length > 0
        ? Math.floor(
            (Date.now() - new Date(matchingDatasets[0].dataset.createdAt).getTime()) /
            (1000 * 60 * 60 * 24)
        )
        : null;
 ...
 */
export const canAnswerQuery = async (
    query: string,
    targetProfile?: string,
    platform?: DatasetPlatform
): Promise<boolean> => {
 ...
 */
export const getRequiredDataset = async (
    query: string,
    targetProfile: string,
    platform: DatasetPlatform
): Promise<{
    ...
    const datasetSuggestion = result.suggestedApproaches.find(
        s => s.requiredDataset
    );
