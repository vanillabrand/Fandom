...
export const analyzeVisualContent = async (
  ...
  brands: Array<{ name: string; confidence: number; imageUrls: string[] }>;
  aestheticTags: string[];
  vibeDescription: string;
  colorPalette: string[];
  products: Array<{ category: string; description: string; imageUrls: string[] }>;
}> => {
  ...
  try {
          ...
          if (result) {
            ...
            if (result.brands && Array.isArray(result.brands)) {
              result.brands = result.brands.map((brand: any) => {
                ...
                const urls = indices.map((idx: number) => {
    ...
    const aggregated = {
      brands: [] as Array<{ name: string; confidence: number; imageUrls: string[] }>,
      aestheticTags: [] as string[],
      vibeDescription: '',
      colorPalette: [] as string[],
      products: [] as Array<{ category: string; description: string; imageUrls: string[] }>
    };

    // Merge brands (deduplicate by name)
    const brandMap = new Map<string, { name: string; confidence: number; imageUrls: string[] }>();
    ...
    const productMap = new Map<string, { category: string; description: string; imageUrls: string[] }>();
    allResults.forEach(result => {
      if (result.products) {
...
export const analyzeFandomDeepDive = async (
  ...
  if (Array.isArray(richContext)) {
    richContext.forEach(p => {
      ...
      if (key) profileMap.set(key, { ...p, _source: 'primary' }); // Mark as primary

      // 2. Flatten Related Profiles
      if (p.relatedProfiles && Array.isArray(p.relatedProfiles)) {
        p.relatedProfiles.forEach((rp: any) => {
  ...
  try {
    ...
    const dataContext = scrapedData.slice(0, contextLimit).map(item => {
      ...
      if (item.relatedProfiles && Array.isArray(item.relatedProfiles)) {
        ...
        networkSignals += ` [Related: ${item.relatedProfiles.map((p: any) => {
    ...
    if (response && response.text) {
      ...
      if (rawJson) {
        ...
        if (unpackedJson.analytics?.topContent && Array.isArray(unpackedJson.analytics.topContent)) {
          unpackedJson.analytics.topContent = unpackedJson.analytics.topContent.map((item: any) => proxyMediaFields(item));
        }
        ...
        if (profileMap.size > 0) {
          ...
          const hydrateNode = (obj: any) => {
            ...
            if (match) {
              ...
              if (!obj.data.profilePicUrl && match.profilePicUrl) obj.data.profilePicUrl = match.profilePicUrl;
              ...
              if (match.fullName && !obj.label) obj.label = match.fullName; // Use full name as label if missing

              // Add provenance if available
              if (match.source) obj.data.citation = `Derived from ${match.source}`;
              if (match.derivedFrom) obj.data.citation = `Related to @${match.derivedFrom}`;
