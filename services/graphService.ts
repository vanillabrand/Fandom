...
import { Dataset, Node, Link } from '../types.js';

/**
 * Subject Matter Graph Service
 * Transforms raw social data (posts, comments) into a Topic/Entity Graph.
 */

// Define the structure of a Topic/Entity Node
 ...
 */
export const transformToSubjectGraph = (dataset: Dataset): { nodes: Node[], links: Link[] } => {

    // 1. Check for Pre-computed Analytics (Preferred)
    const analytics = (dataset as any).analytics || {};
...
const buildGraphFromAiMatches = (dataset: Dataset, matches: any[]): { nodes: Node[], links: Link[] } => {
    const nodes: Node[] = [];
    const links: Link[] = [];
    ...
    const uniqueTopics = new Map<string, { category: string, sentimentSum: number, count: number, originalLabel: string }>();
        ...
        if (m.username) {
            ...
            if (addedNodeIds.has(topicNodeId)) {
                ...
                if (!addedNodeIds.has(userId)) {
                    ...
                    const userRecord = dataset.data.find((r: any) =>
                        (r.username === m.username || r.ownerUsername === m.username)
                    );
...
const buildGraphHeuristically = (dataset: Dataset): { nodes: Node[], links: Link[] } => {
    const nodes: Node[] = [];
    const links: Link[] = [];
